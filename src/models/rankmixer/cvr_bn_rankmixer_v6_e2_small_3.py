# -*- coding: utf-8 -*-
# RankMixer v6-E2-Small-3：由 cvr_bn_rankmixer_v6_e2_small.py 发展的三层主干版本。
# 1. 输入塔保持 Base/v6 的三桶 BN、Hierarchical SENet 和冻结的 31 组语义字段映射。
# 2. 31 个 Local Token 与 1 个 Global Token 均投影为 D=256；投影后、三个 RankMixer
#    Block 的 PreNorm 位置以及最终输出全部保留 v6 RMSNorm。
# 3. 主干保持 H=T=32、L=3、Mixing/Reverting 和 M=704 的双 Per-token SwiGLU。
# 4. 保持 PureFlat 单路读出；最终
#    [B,32,256] 直接展平为 [B,8192]，进入与 v6 相同的 [2048,2048,256] 单任务头。
# 5. Dense 可训练参数量为 137,081,957；该数值仅用于防止实现漂移，不作为实验约束；
...
# 7. 相对 Small 仅增加一个同构 RankMixer Block；L=2→3 会改变容量，
#    不承诺预测/AUC 等价。Token 构造的优化/参考路径开关保持不变。
...
class MLPModel(ModelBase):
    ...
    _EXPECTED_DENSE_TRAINABLE_PARAMS = 137081957
    ...
    def __init__(self, **_kwargs):
        ...
        # RankMixer v6-E2-Small-3: 31 local tokens + 1 global token, H=T=32, D=256, L=3.
        ...
        if not self.use_rankmixer:
            raise ValueError('RankMixer v6-E2-Small-3 requires use_rankmixer=true')
        if self.embedding_size != 17:
            raise ValueError('RankMixer v6-E2-Small-3 requires embedding_size=17, got {}'.format(self.embedding_size))
        ...
        if self.rm_head_num != self.rm_token_num:
            raise ValueError('RankMixer v6-E2-Small-3 requires rm_head_num == rm_token_num')
        ...
        if self.rm_norm_type != 'rms_norm':
            raise ValueError(
                'RankMixer v6-E2-Small-3 requires rm_norm_type=rms_norm, got {}'.format(
                    self.rm_norm_type
                )
            )
        if self.rm_rms_epsilon <= 0.0:
            raise ValueError('RankMixer v6-E2-Small-3 requires rm_rms_epsilon > 0')
        if self.rm_readout_type != 'pure_flat':
            raise ValueError('RankMixer v6-E2-Small-3 requires rm_readout_type=pure_flat')
        ...
        if nonempty_unsupported:
            raise ValueError(
                'RankMixer v6-E2-Small-3 accepts only common/item/creative; '
        ...
        for arg_name, (actual_value, required_value) in required_architecture.items():
            if actual_value != required_value:
                raise ValueError(
                    'RankMixer v6-E2-Small-3 requires {}={}, got {}'.format(
                        arg_name, required_value, actual_value
                    )
                )
        if tuple(field_counts) != self._EXPECTED_FIELD_COUNTS:
            raise ValueError(
                'RankMixer v6-E2-Small-3 requires field counts={}, got {}'.format(
                    self._EXPECTED_FIELD_COUNTS,
                    tuple(field_counts),
                )
            )
        if self.cvr_layers != [2048, 2048, 256]:
            raise ValueError('RankMixer v6-E2-Small-3 requires cvr_layers=[2048, 2048, 256]')
        if not self.use_senet or not self.use_senet_bn or not self.batch_norm:
            raise ValueError(
                'RankMixer v6-E2-Small-3 requires use_senet/use_senet_bn/batch_norm all true'
            )
        if self.use_mlp_gate:
            raise ValueError('RankMixer v6-E2-Small-3 requires use_mlp_gate=false for a single task path')
        if self.rm_token_proj_act != 'gelu_2' or self.mlp_act_type != 'gelu_2':
            raise ValueError('RankMixer v6-E2-Small-3 requires GELU2 projections and task head')
        if abs(self.rm_down_init_scale - 0.01) > 1e-12:
            raise ValueError('RankMixer v6-E2-Small-3 requires rm_down_init_scale=0.01')
        ...
        if self.rm_dense_trainable_param_count != self.rm_expected_dense_trainable_params:
            raise ValueError(
                'RankMixer v6-E2-Small-3 dense trainable parameter count={}, expected={}'.format(
                    self.rm_dense_trainable_param_count,
                    self.rm_expected_dense_trainable_params,
                )
            )
        logging.info(
            'RankMixer v6-E2-Small-3: group_version=%s, fields=%s, local_bucket_tokens=%s, '
        ...
        logging.info(
            'RankMixer v6-E2-Small-3 tokenization: optimized=%s; '
    ...
    def _validate_semantic_feature_groups(self):
        ...
        for bucket_name in self._BUCKET_NAMES:
            ...
            if actual_group_sizes != expected_group_sizes:
                raise ValueError(
                    'RankMixer v6-E2-Small-3 group sizes mismatch for {}: actual={}, expected={}'.format(
                        bucket_name,
                        actual_group_sizes,
            ...
            if checksum != expected_checksum:
                raise ValueError(
                    'RankMixer v6-E2-Small-3 group checksum mismatch for {}: actual={}, expected={}'.format(
                        bucket_name,
                        checksum,
                        expected_checksum,
                    )
            ...
            logging.info(
                'RankMixer v6-E2-Small-3 frozen groups %s: version=%s, checksum=%s, groups=%s',
                bucket_name,
                self._GROUP_VERSION,
                checksum,
    ...
    def _calculate_dense_trainable_params(self):
        ...
        # Local/Block/Final use per-token gamma while Global uses shared gamma.
        local_token_params = (
    ...
    def _verify_graph_dense_trainable_params(self, dense_scope):
        ...
        for variable in dense_variables:
            ...
            if any(dimension is None for dimension in shape):
                raise ValueError(
                    'RankMixer v6-E2-Small-3 dense variable has unknown shape: {} {}'.format(
                        variable.op.name,
                        shape,
                    )
                )
            ...
            if log_manifest:
                logging.info(
                    'RankMixer v6-E2-Small-3 dense variable: name=%s shape=%s params=%d',
                    variable.op.name,
                    shape,
                    variable_total,
                )
        ...
        if actual_total != self.rm_expected_dense_trainable_params:
            raise ValueError(
                'RankMixer v6-E2-Small-3 graph dense trainable parameters={}, expected={}'.format(
                    actual_total,
                    self.rm_expected_dense_trainable_params,
                )
            )
    ...
    def _semantic_tokenize(self, bucket_field_maps, export):
        ...
        with tf.variable_scope(
            ...
            if any(token is None for token in tokens):
                raise ValueError('RankMixer v6-E2-Small-3 failed to project every local token')
        ...
        for token_index, (bucket_name, group_name, feature_ids, _, input_dim) in enumerate(token_specs):
            logging.info(
                'RankMixer v6-E2-Small-3 local token %d: bucket=%s, name=%s, fields=%d, input_dim=%d, D=%d',
                token_index,
                bucket_name,
                group_name,
                len(feature_ids),
    ...
    def _build_global_token(self, bucket_tensors, export):
        ...
        logging.info(
            'RankMixer v6-E2-Small-3 global token: input_dim=%d -> %d -> %d',
            input_dim,
            self.rm_hidden_dim,
            self.rm_hidden_dim,
        )
    ...
    def _rm_mix_tokens(self, inputs):
        ...
        if token_num * head_dim != self.rm_hidden_dim:
            raise ValueError('RankMixer v6-E2-Small-3 requires H=T so mixed dimension remains D')
        return mixed
    ...
    def _rm_block(self, inputs, block_idx, export):
        ...
        logging.info(
            'RankMixer v6-E2-Small-3 block %d: input=%s, mixed=%s, reverted=%s, output=%s',
            block_idx,
            inputs.get_shape(),
            mixed.get_shape(),
            reverted.get_shape(),
    ...
    def model_fn(self, features, labels, timestamps=None, mode="train", export=False):
        ...
        with tf.variable_scope(
            ...
            for name in self._BUCKET_NAMES:
                ...
                if any(dim != self.embedding_size for dim in field_dims):
                    raise ValueError(
                        'RankMixer v6-E2-Small-3 expects every {} field embedding dim to equal {}, got {}'.format(
                            name,
                            self.embedding_size,
        ...
        logging.info(
            "RankMixer v6-E2-Small-3 output: input_tokens=%s, hidden_tokens=%s, final_tokens=%s, "
