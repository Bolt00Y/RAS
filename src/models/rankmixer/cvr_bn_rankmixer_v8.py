# -*- coding: utf-8 -*-
# RankMixer v8 基于已成功运行的 v6 完整实现升级：
# 1. 完整保留 v6 的 31 个语义硬编码 Local Token、1 个 Raw Global Token、H=T=32、D=512、
#    L=2、Mixing/Reverting、PreNorm + RMSNorm、SENet、增强读出和单一 first-CVR BCE。
# 2. 在 SENet 后、语义 Token 化前加入两层 Masked Low-Rank DCN：输入维度 20,978、
#    low-rank r=500、mask bottleneck k=250。Mask 输出层使用小初始化且 bias=1，
#    使 Dense 冷启动时先近似稳定的低秩 DCN，再学习样本相关交叉方向。
# 3. 31 个 Local Token 从 ML-DCN 输出按原字段坐标切分；Global Token 仍从未交叉的
#    post-BN/SENet 全字段表示构建，保留与显式交叉路径互补的原始全局视图。
# 4. Per-token SwiGLU hidden 从 v6 的 M=704 调整为 M=512，使固定 Dense 可训练参数量为
#    192,242,606（约 192.24M），低于 200M 上限。
# 5. 最终仍 Concat Final Global、Global-conditioned Pool 和 Gated Flatten Route，
#    再使用 [2048, 2048, 256] 单路径任务头；不同语义读出不做无条件加和。
# 6. 本文件是 v8 的完整独立实现，不导入、不继承其他 RankMixer 版本。
# 7. 任务头变量 scope 中保留 v5 稳定名称，仅用于保持现有 warm-up/checkpoint 工具兼容。
import os
import math
import hashlib
...
class MLPModel(ModelBase):
    ...
    _GROUP_VERSION = 'rankmixer_v8_semantic_balanced_v1'
    _EXPECTED_FIELD_COUNTS = (385, 835, 14)
    _EXPECTED_DENSE_TRAINABLE_PARAMS = 192242606
    ...
    def __init__(self, **_kwargs):
        ...
        # RankMixer v8: 31 local tokens + 1 raw global token, H=T=32, D=512, L=2.
        ...
        # Full-field Masked Low-Rank DCN before semantic tokenization.
        self.rm_ml_dcn_layer_num = int(_kwargs.get('rm_ml_dcn_layer_num', 2))
        self.rm_ml_dcn_rank = int(_kwargs.get('rm_ml_dcn_rank', 500))
        self.rm_ml_dcn_mask_hidden_dim = int(_kwargs.get('rm_ml_dcn_mask_hidden_dim', 250))
        self.rm_ml_dcn_mask_down_init_scale = float(
            _kwargs.get('rm_ml_dcn_mask_down_init_scale', 0.01)
        )
        self.rm_ml_dcn_mask_output_bias_init = float(
            _kwargs.get('rm_ml_dcn_mask_output_bias_init', 1.0)
        )

        if not self.use_rankmixer:
            raise ValueError('RankMixer v8 requires use_rankmixer=true')
        if self.embedding_size != 17:
            raise ValueError('RankMixer v8 requires embedding_size=17, got {}'.format(self.embedding_size))
        ...
        if self.rm_head_num != self.rm_token_num:
            raise ValueError('RankMixer v8 requires rm_head_num == rm_token_num')
        ...
        if self.rm_ml_dcn_layer_num <= 0:
            raise ValueError('rm_ml_dcn_layer_num must be positive')
        if self.rm_ml_dcn_rank <= 0:
            raise ValueError('rm_ml_dcn_rank must be positive')
        if self.rm_ml_dcn_mask_hidden_dim <= 0:
            raise ValueError('rm_ml_dcn_mask_hidden_dim must be positive')
        if self.rm_ml_dcn_mask_down_init_scale <= 0.0:
            raise ValueError('rm_ml_dcn_mask_down_init_scale must be positive')
        ...
        required_architecture = {
            ...
            'rm_flatten_dim': (self.rm_flatten_dim, 512),
            'rm_ml_dcn_layer_num': (self.rm_ml_dcn_layer_num, 2),
            'rm_ml_dcn_rank': (self.rm_ml_dcn_rank, 500),
            'rm_ml_dcn_mask_hidden_dim': (self.rm_ml_dcn_mask_hidden_dim, 250),
        }
        for arg_name, (actual_value, required_value) in required_architecture.items():
            if actual_value != required_value:
                raise ValueError(
                    'RankMixer v8 requires {}={}, got {}'.format(
                        arg_name,
                        required_value,
                        actual_value,
                    )
        ...
        if self.cvr_layers != [2048, 2048, 256]:
            raise ValueError(
                'RankMixer v8 requires cvr_layers=[2048, 2048, 256], got {}'.format(
                    self.cvr_layers
                )
            )
        if not self.use_senet or not self.use_senet_bn:
            raise ValueError('RankMixer v8 requires use_senet=true and use_senet_bn=true')
        if not self.batch_norm:
            raise ValueError('RankMixer v8 requires batch_norm=true for the task head')
        if self.rm_token_proj_act != 'gelu_2' or self.mlp_act_type != 'gelu_2':
            raise ValueError('RankMixer v8 requires GELU2 token projections and task head')
        if abs(self.rm_down_init_scale - 0.01) > 1e-12:
            raise ValueError('RankMixer v8 requires rm_down_init_scale=0.01')
        if abs(self.rm_flatten_gate_init + 2.0) > 1e-12:
            raise ValueError('RankMixer v8 requires rm_flatten_gate_init=-2.0')
        if abs(self.rm_ml_dcn_mask_down_init_scale - 0.01) > 1e-12:
            raise ValueError('RankMixer v8 requires rm_ml_dcn_mask_down_init_scale=0.01')
        if abs(self.rm_ml_dcn_mask_output_bias_init - 1.0) > 1e-12:
            raise ValueError('RankMixer v8 requires rm_ml_dcn_mask_output_bias_init=1.0')
        ...
        if nonempty_unsupported:
            raise ValueError(
                'RankMixer v8 accepts only common/item/creative; '
        ...
        if tuple(field_counts) != self._EXPECTED_FIELD_COUNTS:
            raise ValueError(
                'RankMixer v8 requires field_counts={}, got {}'.format(
        ...
        if self.rm_dense_trainable_param_count != self._EXPECTED_DENSE_TRAINABLE_PARAMS:
            raise ValueError(
                'RankMixer v8 dense trainable parameter count={}, expected={}'.format(
        ...
        if self.rm_dense_trainable_param_count >= self._DENSE_TRAINABLE_PARAM_LIMIT:
            raise ValueError(
                'RankMixer v8 dense trainable parameter count={} must be below {}'.format(
                    self.rm_dense_trainable_param_count,
        ...
        logging.info(
            'RankMixer v8: group_version=%s, fields=%s, local_bucket_tokens=%s, '
            'T=%d, H=%d, D=%d, L=%d, swiglu_hidden=%d, down_init_scale=%s, rms_eps=%s, '
            'ml_dcn_layers=%d, ml_dcn_rank=%d, ml_dcn_mask_hidden=%d, mask_down_init=%s, '
            'mask_output_bias_init=%s, token_proj_act=%s, senet=%s, pool_query_dim=%d, '
            'flatten_dim=%d, expected_dense_trainable_params=%d, dense_param_limit=%d',
            self._GROUP_VERSION,
            field_counts,
            ...
            self.rm_rms_epsilon,
            self.rm_ml_dcn_layer_num,
            self.rm_ml_dcn_rank,
            self.rm_ml_dcn_mask_hidden_dim,
            self.rm_ml_dcn_mask_down_init_scale,
            self.rm_ml_dcn_mask_output_bias_init,
    ...
    def _build_semantic_feature_groups():
        """Return the frozen v8 semantic-balanced field groups."""
    ...
    def _validate_semantic_feature_groups(self):
        ...
        for bucket_name in self._BUCKET_NAMES:
            ...
            if actual_group_sizes != expected_group_sizes:
                raise ValueError(
                    'RankMixer v8 group sizes mismatch for {}: actual={}, expected={}'.format(
                        bucket_name,
                        actual_group_sizes,
            ...
            if checksum != expected_checksum:
                raise ValueError(
                    'RankMixer v8 group checksum mismatch for {}: actual={}, expected={}'.format(
                        bucket_name,
                        checksum,
                        expected_checksum,
                    )
            ...
            logging.info(
                'RankMixer v8 frozen groups %s: version=%s, checksum=%s, groups=%s',
                bucket_name,
                self._GROUP_VERSION,
                checksum,
    ...
    def _calculate_dense_trainable_params(self):
        ...
        ml_dcn_layer_params = (
            input_dim * self.rm_ml_dcn_rank
            + self.rm_ml_dcn_rank
            + input_dim * self.rm_ml_dcn_mask_hidden_dim
            + self.rm_ml_dcn_mask_hidden_dim
            + self.rm_ml_dcn_mask_hidden_dim * self.rm_ml_dcn_rank
            + self.rm_ml_dcn_rank
            + self.rm_ml_dcn_rank * input_dim
            + input_dim
            + 2 * input_dim
        )
        ml_dcn_params = self.rm_ml_dcn_layer_num * ml_dcn_layer_params
        ...
        task_head_params = 0
        task_input_dim = 2 * self.rm_hidden_dim + self.rm_flatten_dim
        for layer_size in self.cvr_layers:
            task_head_params += task_input_dim * layer_size + layer_size
            task_head_params += 2 * layer_size
            task_input_dim = layer_size
        task_head_params += task_input_dim + 1
    ...
    def _ml_dcn_layer(self, x0, inputs, layer_idx, export):
        """One full-field ML-DCN layer with an identity-centered low-rank mask."""
        input_dim = inputs.shape[-1].value
        x0_dim = x0.shape[-1].value
        if input_dim is None or x0_dim is None:
            raise ValueError('ML-DCN input dimensions must be statically known')
        if input_dim != x0_dim:
            raise ValueError(
                'ML-DCN x0/input dimension mismatch: {} vs {}'.format(
                    x0_dim,
                    input_dim,
                )
            )

        mask_down_initializer = tf.random_normal_initializer(
            stddev=(
                self.rm_ml_dcn_mask_down_init_scale
                / math.sqrt(self.rm_ml_dcn_mask_hidden_dim)
            )
        )
        ...
        with tf.variable_scope(
            ...
            rank_projection = tf.contrib.layers.fully_connected(
                inputs=inputs,
                num_outputs=self.rm_ml_dcn_rank,
                activation_fn=tf.identity,
                weights_initializer=self.get_init(input_dim),
                biases_initializer=tf.zeros_initializer(),
                weights_regularizer=regularizer,
                scope='rank_projection',
            )
            mask_hidden = tf.contrib.layers.fully_connected(
                inputs=inputs,
                num_outputs=self.rm_ml_dcn_mask_hidden_dim,
                activation_fn=tf.nn.relu,
                weights_initializer=self.get_init(input_dim),
                biases_initializer=tf.zeros_initializer(),
                weights_regularizer=regularizer,
                scope='mask_aggregation',
            )
            mask = tf.contrib.layers.fully_connected(
                inputs=mask_hidden,
                num_outputs=self.rm_ml_dcn_rank,
                activation_fn=tf.identity,
                weights_initializer=mask_down_initializer,
                biases_initializer=tf.constant_initializer(
                    self.rm_ml_dcn_mask_output_bias_init
                ),
                weights_regularizer=regularizer,
                scope='mask_projection',
            )
            masked_rank = tf.multiply(rank_projection, mask, name='masked_rank')
            restored = tf.contrib.layers.fully_connected(
                inputs=masked_rank,
                num_outputs=input_dim,
                activation_fn=tf.identity,
                weights_initializer=self.get_init(self.rm_ml_dcn_rank),
                biases_initializer=tf.zeros_initializer(),
                weights_regularizer=regularizer,
                scope='rank_restore',
            )
            cross_update = tf.multiply(x0, restored, name='full_field_cross')
            residual = tf.add(inputs, cross_update, name='residual_add')
            output = layer_norm(
                residual,
                name='layer_norm',
                export=export,
        ...
        logging.info(
            'RankMixer v8 ML-DCN layer %d: input=%s, rank=%d, mask_hidden=%d, output=%s',
            layer_idx,
            inputs.get_shape(),
            self.rm_ml_dcn_rank,
            self.rm_ml_dcn_mask_hidden_dim,
        ...
        return output, mask, cross_update

    def _ml_dcn_stack(self, inputs, export):
        """Apply two ML-DCN layers and expose read-only stability diagnostics."""
        input_dim = inputs.shape[-1].value
        expected_input_dim = sum(self._EXPECTED_FIELD_COUNTS) * self.embedding_size
        if input_dim != expected_input_dim:
            raise ValueError(
                'RankMixer v8 ML-DCN expects input_dim={}, got {}'.format(
                    expected_input_dim,
                    input_dim,
                )
            )
        ...
        with tf.variable_scope(
            ...
            for layer_idx in range(self.rm_ml_dcn_layer_num):
                layer_input = output
                output, mask, cross_update = self._ml_dcn_layer(
                    inputs,
                    layer_input,
                    layer_idx,
                    export,
                ...
                masks.append(mask)
                input_rms = tf.sqrt(
                    tf.reduce_mean(tf.square(layer_input), axis=-1) + self.rm_rms_epsilon
                )
                cross_rms = tf.sqrt(
                    tf.reduce_mean(tf.square(cross_update), axis=-1) + self.rm_rms_epsilon
                )
                cross_rms_ratios.append(
                    tf.reduce_mean(cross_rms / tf.maximum(input_rms, self.rm_rms_epsilon))
                )

        stacked_masks = tf.stack(masks, axis=1, name='stacked_masks')
        mask_mean = tf.reduce_mean(stacked_masks)
        self.rm_ml_dcn_mask_mean = mask_mean
        self.rm_ml_dcn_mask_std = tf.sqrt(
            tf.reduce_mean(tf.square(stacked_masks - mask_mean)) + self.rm_rms_epsilon
        )
        self.rm_ml_dcn_mask_near_zero_ratio = tf.reduce_mean(
            tf.cast(tf.less(tf.abs(stacked_masks), 0.05), tf.float32)
        )
        self.rm_ml_dcn_mask_large_ratio = tf.reduce_mean(
            tf.cast(tf.greater(tf.abs(stacked_masks), 5.0), tf.float32)
        )
        self.rm_ml_dcn_cross_rms_ratios = tf.stack(
            cross_rms_ratios,
            axis=0,
            name='cross_rms_ratios',
        )
        return output
    ...
    def _semantic_tokenize(self, bucket_field_maps):
        ...
        with tf.variable_scope(
            ...
            if any(token is None for token in tokens):
                raise ValueError('RankMixer v8 failed to project every local token')
        ...
        for token_index, (bucket_name, group_name, feature_ids, _, input_dim) in enumerate(token_specs):
            logging.info(
                'RankMixer v8 local token %d: bucket=%s, name=%s, fields=%d, input_dim=%d, D=%d',
                token_index,
                bucket_name,
                group_name,
                len(feature_ids),
    ...
    def _build_global_token(self, bucket_tensors):
        ...
        logging.info(
            'RankMixer v8 raw global token: input_dim=%d -> %d -> %d',
            input_dim,
            self.rm_hidden_dim,
            self.rm_hidden_dim,
        )
    ...
    def _rm_mix_tokens(self, inputs):
        ...
        if token_num * head_dim != self.rm_hidden_dim:
            raise ValueError('RankMixer v8 requires H=T so mixed dimension remains D')
        return mixed

    def _rm_revert_tokens(self, mixed):
        """Exact inverse of _rm_mix_tokens for the v8 H=T layout."""
    ...
    def _rm_block(self, inputs, block_idx):
        ...
        logging.info(
            'RankMixer v8 block %d: input=%s, mixed=%s, reverted=%s, output=%s',
            block_idx,
            inputs.get_shape(),
            mixed.get_shape(),
            reverted.get_shape(),
    ...
    def model_fn(self, features, labels, timestamps=None, mode="train", export=False):
        ...
        with tf.variable_scope("Cvr-task-part", reuse=tf.AUTO_REUSE, partitioner=self.partitioner):
            ...
            raw_full_input = tf.concat(
                [bucket_tensors[name] for name in self._BUCKET_NAMES],
                axis=-1,
                name='rm_raw_full_input',
            )
            expected_input_dim = sum(self._EXPECTED_FIELD_COUNTS) * self.embedding_size
            if raw_full_input.shape[-1].value != expected_input_dim:
                raise ValueError(
                    'RankMixer v8 raw full input must be {}, got {}'.format(
                        expected_input_dim,
                        raw_full_input.shape[-1].value,
                    )
                )
            ml_dcn_output = self._ml_dcn_stack(raw_full_input, export)
            ml_dcn_output.set_shape([None, expected_input_dim])

            bucket_dims = [bucket_tensors[name].shape[-1].value for name in self._BUCKET_NAMES]
            if any(dim is None for dim in bucket_dims):
                raise ValueError('RankMixer v8 bucket dimensions must be statically known')
            if sum(bucket_dims) != expected_input_dim:
                raise ValueError(
                    'RankMixer v8 bucket dimensions sum to {}, expected {}'.format(
                        sum(bucket_dims),
                        expected_input_dim,
                    )
                )
            crossed_bucket_values = tf.split(ml_dcn_output, bucket_dims, axis=-1)
            crossed_bucket_tensors = dict(zip(self._BUCKET_NAMES, crossed_bucket_values))
            ...
            for name in self._BUCKET_NAMES:
                ...
                if any(dim != self.embedding_size for dim in field_dims):
                    raise ValueError(
                        'RankMixer v8 expects every {} field embedding dim to equal {}, got {}'.format(
                            name,
                            self.embedding_size,
                            sorted(set(field_dims)),
                        )
                    )
                field_tensors = tf.split(crossed_bucket_tensors[name], field_dims, axis=-1)
            ...
            # Keep the Global Token on the raw post-BN/SENet view; Local Tokens use ML-DCN.
        ...
        logging.info(
            "RankMixer v8 output: ml_dcn_output=%s, input_tokens=%s, hidden_tokens=%s, "
            "final_tokens=%s, context=%s",
            ml_dcn_output.get_shape(),
