...
#
# The fixed post-SENet input is processed by the exact Base 2xDCNM500 tower.
# Raw/Cross semantic tokens and a 512-d DCNM shortcut are then trained end to end
# through one [2048, 2048, 256] CVR head. This module intentionally uses the same
# TensorFlow 1.x/Flood/Cayman APIs as the existing production models.
import os
import math
import hashlib
...
try:
    ...
    logging.info('layer_norm_for_train import error')


class MLPModel(ModelBase):
    _BUCKET_NAMES = ('common', 'item', 'creative')
    _EXPECTED_FIELD_COUNTS = (385, 835, 14)
    _EXPECTED_DENSE_TRAINABLE_PARAMS = 199445658
    _DENSE_TRAINABLE_PARAM_LIMIT = 200000000
    _GROUP_VERSION = 'rankmixer_v9_semantic_balanced_v1'
    _GROUP_CHECKSUMS = {
    ...
    def __init__(self, **_kwargs):
        ...
        # RankMixer v9: 31 local tokens + 1 global token, H=T=32, D=512, L=2.
        ...
        self.rm_swiglu_hidden_dim = int(_kwargs.get('rm_swiglu_hidden_dim', 512))
        ...
        self.rm_flatten_dim = int(_kwargs.get('rm_flatten_dim', 256))
        self.rm_flatten_gate_init = float(_kwargs.get('rm_flatten_gate_init', -2.0))

        # Exact Base DCNM configuration.
        self.dcnm_layer = int(_kwargs.get('dcnm_layer', 500))
        self.cross_num = int(_kwargs.get('cross_num', 2))
        self.use_cross_act = bool(_kwargs.get('use_cross_act', False))
        self.layer_norm_opt = bool(_kwargs.get('layer_norm_opt', True))
        self.rm_dcnm_shortcut_dim = int(
            _kwargs.get('rm_dcnm_shortcut_dim', 512)
        )

        if not self.use_rankmixer:
            raise ValueError('RankMixer v9 requires use_rankmixer=true')
        if self.embedding_size != 17:
            raise ValueError('RankMixer v9 requires embedding_size=17, got {}'.format(self.embedding_size))
        ...
        if self.rm_head_num != self.rm_token_num:
            raise ValueError('RankMixer v9 requires rm_head_num == rm_token_num')
        ...
        if self.dcnm_layer <= 0 or self.cross_num <= 0:
            raise ValueError('dcnm_layer and cross_num must be positive')
        if self.rm_dcnm_shortcut_dim <= 0:
            raise ValueError('rm_dcnm_shortcut_dim must be positive')
        ...
        if nonempty_unsupported:
            raise ValueError(
                'RankMixer v9 accepts only common/item/creative; '
        ...
        if tuple(field_counts) != self._EXPECTED_FIELD_COUNTS:
            raise ValueError(
                'RankMixer v9 requires field_counts={}, got {}'.format(
        ...
        required_architecture = {
            ...
            'dcnm_layer': (self.dcnm_layer, 500),
            'cross_num': (self.cross_num, 2),
            'rm_token_num': (self.rm_token_num, 32),
            'rm_local_token_num': (self.rm_local_token_num, 31),
            'rm_hidden_dim': (self.rm_hidden_dim, 512),
            'rm_layer_num': (self.rm_layer_num, 2),
            'rm_head_num': (self.rm_head_num, 32),
            'rm_swiglu_hidden_dim': (self.rm_swiglu_hidden_dim, 512),
            'rm_pool_query_dim': (self.rm_pool_query_dim, 128),
            'rm_flatten_dim': (self.rm_flatten_dim, 256),
            'rm_dcnm_shortcut_dim': (self.rm_dcnm_shortcut_dim, 512),
        }
        for arg_name, (actual_value, required_value) in required_architecture.items():
            if actual_value != required_value:
                raise ValueError(
                    'RankMixer v9 requires {}={}, got {}'.format(
                        arg_name, required_value, actual_value
                    )
                )
        if self.cvr_layers != [2048, 2048, 256]:
            raise ValueError('RankMixer v9 requires cvr_layers=[2048, 2048, 256]')
        if not self.use_senet or not self.use_senet_bn or not self.batch_norm:
            raise ValueError(
                'RankMixer v9 requires use_senet/use_senet_bn/batch_norm all true'
            )
        if self.use_mlp_gate:
            raise ValueError('RankMixer v9 requires use_mlp_gate=false for Base parity')
        if self.use_cross_act or not self.layer_norm_opt:
            raise ValueError(
                'RankMixer v9 requires use_cross_act=false and layer_norm_opt=true'
            )
        if self.rm_token_proj_act != 'gelu_2' or self.mlp_act_type != 'gelu_2':
            raise ValueError('RankMixer v9 requires GELU2 projections and heads')
        if abs(self.rm_down_init_scale - 0.01) > 1e-12:
            raise ValueError('RankMixer v9 requires rm_down_init_scale=0.01')
        if abs(self.rm_flatten_gate_init + 2.0) > 1e-12:
            raise ValueError('RankMixer v9 requires rm_flatten_gate_init=-2.0')

        self.rm_dense_trainable_param_count = self._calculate_dense_trainable_params()
        if self.rm_dense_trainable_param_count != self._EXPECTED_DENSE_TRAINABLE_PARAMS:
            raise ValueError(
                'RankMixer v9 dense trainable parameter count={}, expected={}'.format(
        ...
        if self.rm_dense_trainable_param_count >= self._DENSE_TRAINABLE_PARAM_LIMIT:
            raise ValueError(
                'RankMixer v9 dense trainable parameters must remain below {}'.format(
                    self._DENSE_TRAINABLE_PARAM_LIMIT
                )
            )

        logging.info(
            'RankMixer v9: group_version=%s, fields=%s, local_bucket_tokens=%s, '
            'T=%d, H=%d, D=%d, L=%d, swiglu_hidden=%d, down_init_scale=%s, rms_eps=%s, '
            'token_proj_act=%s, senet=%s, pool_query_dim=%d, flatten_dim=%d, '
            'dcnm_shortcut_dim=%d, fusion_dim=%d, dense_params=%d',
            self._GROUP_VERSION,
            field_counts,
            ...
            self.rm_flatten_dim,
            self.rm_dcnm_shortcut_dim,
            2 * self.rm_hidden_dim + self.rm_flatten_dim + self.rm_dcnm_shortcut_dim,
            self.rm_dense_trainable_param_count,
        )
    ...
    def _build_semantic_feature_groups():
        """Return the frozen v9 semantic-balanced field groups."""
    ...
    def _validate_semantic_feature_groups(self):
        ...
        for bucket_name in self._BUCKET_NAMES:
            ...
            if actual_group_sizes != expected_group_sizes:
                raise ValueError(
                    'RankMixer v9 group sizes mismatch for {}: actual={}, expected={}'.format(
                        bucket_name,
                        actual_group_sizes,
            ...
            if checksum != expected_checksum:
                raise ValueError(
                    'RankMixer v9 group checksum mismatch for {}: actual={}, expected={}'.format(
                        bucket_name,
                        checksum,
                        expected_checksum,
                    )
            ...
            logging.info(
                'RankMixer v9 frozen groups %s: version=%s, checksum=%s, groups=%s',
                bucket_name,
                self._GROUP_VERSION,
                checksum,
    ...
    def _calculate_dense_trainable_params(self):
        ...
        one_dcnm_layer_params = (
            input_dim * self.dcnm_layer
            + self.dcnm_layer
            + self.dcnm_layer * input_dim
            + input_dim
            + 2 * input_dim
        )
        dcnm_params = self.cross_num * one_dcnm_layer_params

        # Every local token projects concat(raw_group, crossed_group).
        local_token_params = (
            2 * input_dim * self.rm_hidden_dim
        ...
        block_params = self.rm_layer_num * (
            2 * self.rm_token_num * self.rm_hidden_dim
            + 2 * one_swiglu_params
        )
        final_norm_params = self.rm_token_num * self.rm_hidden_dim
        pool_params = 2 * (
            self.rm_hidden_dim * self.rm_pool_query_dim
            + self.rm_pool_query_dim
        )

        flattened_dim = self.rm_local_token_num * self.rm_hidden_dim
        flatten_params = (
            flattened_dim * self.rm_flatten_dim
            + self.rm_flatten_dim
            + self.rm_flatten_dim
            + 1
        )
        ...
        shortcut_params = (
            input_dim * self.rm_dcnm_shortcut_dim
            + self.rm_dcnm_shortcut_dim
            + 2 * self.rm_dcnm_shortcut_dim
        )

        task_head_params = 0
        task_head_input_dim = (
            2 * self.rm_hidden_dim
            + self.rm_flatten_dim
            + self.rm_dcnm_shortcut_dim
        )
    ...
    def _verify_graph_dense_trainable_params(self, dense_scope):
        ...
        for variable in dense_variables:
            ...
            if any(dimension is None for dimension in shape):
                raise ValueError(
                    'RankMixer v9 dense variable has unknown shape: {} {}'.format(
                        variable.op.name,
                        shape,
                    )
                )
            ...
            if log_manifest:
                logging.info(
                    'RankMixer v9 dense variable: name=%s shape=%s params=%d',
                    variable.op.name,
                    shape,
                    variable_total,
                )
        ...
        if actual_total != self._EXPECTED_DENSE_TRAINABLE_PARAMS:
            raise ValueError(
                'RankMixer v9 graph dense trainable parameters={}, expected={}'.format(
                    actual_total,
                    self._EXPECTED_DENSE_TRAINABLE_PARAMS,
                )
            )
    ...
    def build_loss_op(self, mode):
        """Build the single first-CVR BCE used by Base and prior RankMixer runs."""
        labels = tf.reshape(self[f'{mode}_labels'], shape=[-1])
        pred = tf.reshape(self[f'{mode}_pred'], shape=[-1])
        self.loss = tf.reduce_mean(
            tf.losses.log_loss(predictions=pred, labels=labels)
        )
        self.labels_pos_cvr_count = tf.reduce_sum(labels)

    def build_auc_copc_op(self, mode):
        """Build the same main CVR metrics as the Base comparison."""
    ...
    def build_summary(self, mode):
        ...
        pool_entropy_summary = tf.summary.scalar(
            f'{mode}/rm_v9/pool_entropy',
            self.rm_pool_entropy,
        )
        flatten_gate_summary = tf.summary.scalar(
            f'{mode}/rm_v9/flatten_gate',
            self.rm_flatten_gate,
        )
        rankmixer_context_rms_summary = tf.summary.scalar(
            f'{mode}/rm_v9/rankmixer_context_rms',
            self.rm_rankmixer_context_rms,
        )
        dcnm_shortcut_rms_summary = tf.summary.scalar(
            f'{mode}/rm_v9/dcnm_shortcut_rms',
            self.rm_dcnm_shortcut_rms,
        )
        ...
        self.eval_summary = tf.summary.merge(
            [
                ...
                rankmixer_context_rms_summary,
                dcnm_shortcut_rms_summary,
            ],
            name='eval_summary',
        )
    ...
    def test(self, session, worker_id=0, prefix='test', **kwargs):
        ...
        final_auc = RocAucAccum(num_thresholds=2000)
        final_pr_auc = PrAucAccum(num_thresholds=2000)
        final_copc = COPCAccum()
        final_bucket_error = BucketErrorAccum(1000)
        final_sample_count = SampleCntAccum()
        ...
        while True:
            try:
                batch = session.run(fetches, options=self.timeout_options)
                labels = batch['labels']
                ...
                final_auc.update(labels, predictions)
                final_pr_auc.update(labels, predictions)
                final_copc.update(labels, predictions)
                final_bucket_error.update(labels, predictions)
                final_sample_count.update(labels, predictions)

                if 0 < self.test_batch_num < test_cnt:
                    logging.info(
                        'finish test by test_batch_num=%s',
                        self.test_batch_num,
                    )
                    break
                if test_cnt % kwargs.get('test_log_step', 10) == 0:
                    logging.info(
                        'CVR AUC: %.6f; CVR COPC: %.6f',
                        batch['auc'],
                        batch['copc'],
                    )
            except tf.errors.OutOfRangeError as error:
                logging.info('all data set used. %s', error.message)
                break
            except tf.errors.DeadlineExceededError as error:
                logging.error('test step timed out: %s', error.message)
                break
            except tf.errors.InvalidArgumentError as error:
                logging.warning('data error: %s', error.message)
                continue
            except tf.errors.PermissionDeniedError as error:
                logging.error('PermissionDeniedError: %s', str(error))
                break
            except tf.errors.FailedPreconditionError as error:
                logging.error('FailedPreconditionError: %s', str(error))
                break
            except RuntimeError as error:
                logging.warning('runtime error: %s', str(error))
                break

        accum_metrics = {
            'cvr-tower': {
                'roc_auc': final_auc.dump(),
                'copc': final_copc.dump(),
                'pr_auc': final_pr_auc.dump(),
                'bucket_error': final_bucket_error.dump(),
                'sample_cnt': final_sample_count.dump(),
            },
        }
        result = {
            'accum_metrics': accum_metrics,
            'title': (
                'lamb-feature-{}'.format(self.random_feature)
                if self.random_feature else 'rankmixer-v9'
            ),
        }
    ...
    def dcnm_cross_layer(self, inputs, is_train, export):
        """Exact Base 2xDCNM500 cross tower, including scopes and initializers."""
        del is_train
        with tf.variable_scope(
            ...
            if input_dim is None:
                raise ValueError('Base DCNM input dimension must be statically known')

            for layer_idx in range(self.cross_num):
                last_layer_cvr = deep_inputs_cvr
                deep_inputs_cvr = tf.contrib.layers.fully_connected(
                    inputs=deep_inputs_cvr,
                    num_outputs=self.dcnm_layer,
                    activation_fn=(
                        self.get_act_func(self.act_type)
                        if self.use_cross_act else tf.identity
                    ),
                    weights_initializer=tf.random_normal_initializer(
                        stddev=1.0 / math.sqrt(deep_inputs_cvr.shape[1].value)
                    ),
                    weights_regularizer=tf.contrib.layers.l2_regularizer(self.l2_deep),
                    scope='dcnm_cross_layer0_{}'.format(layer_idx),
                )
                deep_inputs_cvr = tf.contrib.layers.fully_connected(
                    inputs=deep_inputs_cvr,
                    ...
                    weights_regularizer=tf.contrib.layers.l2_regularizer(self.l2_deep),
                    scope='dcnm_cross_layer1_{}'.format(layer_idx),
                )
                deep_inputs_cvr = tf.multiply(inputs, deep_inputs_cvr) + last_layer_cvr

                if self.layer_norm_opt:
                    if export:
                        deep_inputs_cvr = tf.contrib.layers.layer_norm(
                            deep_inputs_cvr,
                            center=True,
                            scale=True,
                            activation_fn=None,
                            scope='dcnm_ln_{}'.format(layer_idx),
                            begin_norm_axis=-1,
                            begin_params_axis=-1,
                        )
                    else:
                        if layer_norm_for_train is None:
                            raise ImportError(
                                'RankMixer v9 train graph requires '
                                'cayman.python.layer_norm_for_train for Base parity'
                            )
                        deep_inputs_cvr = layer_norm_for_train(
                            deep_inputs_cvr,
                            scope='dcnm_ln_{}'.format(layer_idx),
                            begin_norm_axis=-1,
                            begin_params_axis=-1,
    ...
    def _semantic_tokenize(
            self,
            raw_bucket_field_maps,
            crossed_bucket_field_maps):
        """Build 31 canonical local tokens from paired raw and DCNM views."""
        token_specs = []
        for bucket_name in self._BUCKET_NAMES:
            for group_name, feature_ids in self.rm_semantic_feature_groups[bucket_name]:
                raw_group = [
                    raw_bucket_field_maps[bucket_name][feature_id]
                    for feature_id in feature_ids
                ]
                crossed_group = [
                    crossed_bucket_field_maps[bucket_name][feature_id]
                    for feature_id in feature_ids
                ]
                raw_input = (
                    raw_group[0]
                    if len(raw_group) == 1
                    else tf.concat(raw_group, axis=-1)
                )
                crossed_input = (
                    crossed_group[0]
                    if len(crossed_group) == 1
                    else tf.concat(crossed_group, axis=-1)
                )
                if raw_input.shape[-1].value != crossed_input.shape[-1].value:
                    raise ValueError(
                        'raw/cross token width mismatch for {}: {} vs {}'.format(
                            group_name,
                            raw_input.shape[-1].value,
                            crossed_input.shape[-1].value,
                        )
                    )
                token_input = tf.concat(
                    [raw_input, crossed_input],
                    axis=-1,
                )
        ...
        with tf.variable_scope(
            ...
            if any(token is None for token in tokens):
                raise ValueError('RankMixer v9 failed to project every local token')
        ...
        for token_index, (bucket_name, group_name, feature_ids, _, input_dim) in enumerate(token_specs):
            logging.info(
                'RankMixer v9 local token %d: bucket=%s, name=%s, fields=%d, input_dim=%d, D=%d',
                token_index,
                bucket_name,
                group_name,
                len(feature_ids),
    ...
    def _build_global_token(self, crossed_input):
        """Encode the complete 20,978-d Base DCNM output as one global token."""
        global_input = crossed_input
        ...
        logging.info(
            'RankMixer v9 global token: input_dim=%d -> %d -> %d',
            input_dim,
            self.rm_hidden_dim,
            self.rm_hidden_dim,
        )
    ...
    def _rm_mix_tokens(self, inputs):
        ...
        if token_num * head_dim != self.rm_hidden_dim:
            raise ValueError('RankMixer v9 requires H=T so mixed dimension remains D')
        return mixed

    def _rm_revert_tokens(self, mixed):
        """Exact inverse of _rm_mix_tokens for the v9 H=T layout."""
    ...
    def _rm_block(self, inputs, block_idx):
        ...
        logging.info(
            'RankMixer v9 block %d: input=%s, mixed=%s, reverted=%s, output=%s',
            block_idx,
            inputs.get_shape(),
            mixed.get_shape(),
            reverted.get_shape(),
    ...
    def _dcnm_shortcut(self, crossed_input, is_train, export):
        """Compress the full Base DCNM output into the 512-d direct path."""
        input_dim = crossed_input.shape[-1].value
        if input_dim is None:
            raise ValueError('DCNM shortcut input dimension must be statically known')
        with tf.variable_scope(
            'rm_dcnm_shortcut',
            reuse=tf.AUTO_REUSE,
            partitioner=self.partitioner,
        ):
            shortcut = tf.contrib.layers.fully_connected(
                inputs=crossed_input,
                num_outputs=self.rm_dcnm_shortcut_dim,
                activation_fn=None,
                weights_initializer=self.get_init(input_dim),
                biases_initializer=tf.zeros_initializer(),
                weights_regularizer=tf.contrib.layers.l2_regularizer(self.l2_deep),
            ...
            shortcut = ModelBase.batch_norm_layer_v2(
                x=shortcut,
                train_phase=is_train,
                scope_bn='bn',
                batch_norm_decay=self.batch_norm_decay,
            ...
            shortcut = self.get_act_func(self.mlp_act_type)(shortcut)
        return shortcut

    def _task_head(self, context, is_train, export):
        """Single end-to-end [2048, 2048, 256] first-CVR prediction tower."""
        hidden = context
        for layer_idx, layer_size in enumerate(self.cvr_layers):
            ...
            hidden = tf.contrib.layers.fully_connected(
                ...
                biases_initializer=tf.zeros_initializer(),
                weights_regularizer=tf.contrib.layers.l2_regularizer(self.l2_deep),
                scope='rm_v9_mlp{}'.format(layer_idx),
            )
            hidden = ModelBase.batch_norm_layer_v2(
                x=hidden,
                train_phase=is_train,
                scope_bn='rm_v9_bn_{}'.format(layer_idx),
                batch_norm_decay=self.batch_norm_decay,
                use_riemann_bn=self.use_riemann_bn,
                export=export,
            )
    ...
    def model_fn(self, features, labels, timestamps=None, mode="train", export=False):
        ...
        # Preserve the exact lookup order used by Base. Semantic token lookup is
        # keyed by field id, so group construction does not reorder the DCNM input.
        bucket_embeddings = {name: [] for name in self._BUCKET_NAMES}
        bucket_feature_ids = {name: [] for name in self._BUCKET_NAMES}
        for index, column in enumerate(
            ...
            if bucket_name is None:
                continue
            if feature_id in bucket_feature_ids[bucket_name]:
                raise ValueError(
                    'duplicated lookup embedding for feature {}'.format(feature_id)
                )
            embedding = sparse_embeddings[index]
            embedding_dim = embedding.shape[-1].value
            if embedding_dim != self.embedding_size:
                raise ValueError(
                    'RankMixer v9 expects embedding dim {} for {}, got {}'.format(
                        self.embedding_size,
                        feature_id,
                        embedding_dim,
                    )
                )
            bucket_feature_ids[bucket_name].append(feature_id)
            bucket_embeddings[bucket_name].append(embedding)
        ...
        for bucket_name in self._BUCKET_NAMES:
            actual_ids = set(bucket_feature_ids[bucket_name])
            if actual_ids != expected_ids[bucket_name]:
                raise ValueError(
                    'lookup field mismatch for {}: missing={}, unknown={}'.format(
                        bucket_name,
                        sorted(expected_ids[bucket_name] - actual_ids),
                        sorted(actual_ids - expected_ids[bucket_name]),
                    )
                )
            if len(bucket_feature_ids[bucket_name]) != self._EXPECTED_FIELD_COUNTS[
                    self._BUCKET_NAMES.index(bucket_name)]:
                raise ValueError(
                    'unexpected {} lookup field count {}'.format(
                        bucket_name,
                        len(bucket_feature_ids[bucket_name]),
                    )
                )
        ...
        with tf.variable_scope(
            ...
            bucket_tensors = dict(zip(self._BUCKET_NAMES, gated_values))
            base_input = tf.concat(
                [bucket_tensors[name] for name in self._BUCKET_NAMES],
                axis=-1,
                name='base_dcnm_input',
            )
            expected_input_dim = sum(self._EXPECTED_FIELD_COUNTS) * self.embedding_size
            if base_input.shape[-1].value != expected_input_dim:
                raise ValueError(
                    'Base DCNM input must be {}, got {}'.format(
                        expected_input_dim,
                        base_input.shape[-1].value,
                    )
                )

            crossed_input = self.dcnm_cross_layer(base_input, is_train, export)

            bucket_dims = [
                bucket_tensors[name].shape[-1].value
                for name in self._BUCKET_NAMES
            ]
            if any(dim is None for dim in bucket_dims):
                raise ValueError('all v9 bucket dimensions must be statically known')
            crossed_bucket_values = tf.split(crossed_input, bucket_dims, axis=-1)
            crossed_bucket_tensors = dict(
                zip(self._BUCKET_NAMES, crossed_bucket_values)
            )
            ...
            for bucket_name in self._BUCKET_NAMES:
                ...
                if any(dim is None for dim in field_dims):
                    raise ValueError(
                        'all {} field dimensions must be statically known'.format(
                            bucket_name
                        )
                    )
                raw_field_tensors = tf.split(
                    bucket_tensors[bucket_name],
                    field_dims,
                    axis=-1,
                )
                crossed_field_tensors = tf.split(
                    crossed_bucket_tensors[bucket_name],
                    field_dims,
                    axis=-1,
                )
                if (
                        len(raw_field_tensors) != len(bucket_feature_ids[bucket_name])
                        or len(crossed_field_tensors) != len(bucket_feature_ids[bucket_name])):
                    raise ValueError(
                        'field tensor count mismatch for {}'.format(bucket_name)
                    )
                raw_bucket_field_maps[bucket_name] = dict(
                    zip(bucket_feature_ids[bucket_name], raw_field_tensors)
                )
                crossed_bucket_field_maps[bucket_name] = dict(
                    zip(bucket_feature_ids[bucket_name], crossed_field_tensors)
                )
            ...
            global_token = self._build_global_token(crossed_input)
            input_tokens = tf.concat(
                [local_tokens, tf.expand_dims(global_token, axis=1)],
                axis=1,
            )
            input_tokens.set_shape([
                None,
                self.rm_token_num,
                self.rm_hidden_dim,
            ])
            ...
            flatten_context, flatten_gate = self._flatten_readout(
                final_local_tokens
            )
            rankmixer_context = tf.concat(
                [global_context, pooled_context, flatten_context],
                axis=-1,
                name='rankmixer_context',
            )
            rankmixer_context.set_shape([
                None,
                2 * self.rm_hidden_dim + self.rm_flatten_dim,
            ])

            dcnm_shortcut = self._dcnm_shortcut(
                crossed_input,
                is_train,
                export,
            )
            fusion_context = tf.concat(
                [rankmixer_context, dcnm_shortcut],
                axis=-1,
                name='rankmixer_dcnm_fusion',
            )
            fusion_context.set_shape([
                None,
                2 * self.rm_hidden_dim
                + self.rm_flatten_dim
                + self.rm_dcnm_shortcut_dim,
            ...
            output = self._task_head(fusion_context, is_train, export)
            logits = tf.clip_by_value(
                tf.reshape(output, shape=[-1], name='{}_logits'.format(mode)),
                -self.clip_val,
                self.clip_val,
            )
            ...
            self.rm_rankmixer_context_rms = tf.sqrt(
                tf.reduce_mean(tf.square(rankmixer_context))
                + self.rm_rms_epsilon
            )
            self.rm_dcnm_shortcut_rms = tf.sqrt(
                tf.reduce_mean(tf.square(dcnm_shortcut))
                + self.rm_rms_epsilon
            )
        ...
        logging.info(
            'RankMixer v9 output: base_input=%s, crossed_input=%s, '
            'input_tokens=%s, hidden_tokens=%s, rankmixer_context=%s, '
            'dcnm_shortcut=%s, fusion_context=%s, '
            'dense_params=%d, graph_dense_params=%d',
            base_input.get_shape(),
            crossed_input.get_shape(),
            input_tokens.get_shape(),
            hidden_tokens.get_shape(),
            rankmixer_context.get_shape(),
            dcnm_shortcut.get_shape(),
            fusion_context.get_shape(),
            self.rm_dense_trainable_param_count,
