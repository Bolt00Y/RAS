...
class MLPModel(ModelBase):
    ...
    def __init__(self, **_kwargs):
        ...
        # RankMixer v6-E2: 31 local tokens + 1 global token, H=T=32, D=512, L=2.
        ...
        if not self.use_rankmixer:
            raise ValueError('RankMixer v6-E2 requires use_rankmixer=true')
        if self.embedding_size != 17:
            raise ValueError('RankMixer v6-E2 requires embedding_size=17, got {}'.format(self.embedding_size))
        ...
        if self.rm_head_num != self.rm_token_num:
            raise ValueError('RankMixer v6-E2 requires rm_head_num == rm_token_num')
        ...
        if self.rm_norm_type != 'rms_norm':
            raise ValueError(
                'RankMixer v6-E2 requires rm_norm_type=rms_norm, got {}'.format(
                    self.rm_norm_type
                )
            )
        if self.rm_rms_epsilon <= 0.0:
            raise ValueError('RankMixer v6-E2 requires rm_rms_epsilon > 0')
        if self.rm_readout_type != 'pure_flat':
            raise ValueError('RankMixer v6-E2 requires rm_readout_type=pure_flat')
        ...
        if nonempty_unsupported:
            raise ValueError(
                'RankMixer v6-E2 accepts only common/item/creative; '
        ...
        if self.use_mlp_gate:
            raise ValueError('RankMixer v6-E2 requires use_mlp_gate=false for a single task path')
        if self.rm_token_proj_act != 'gelu_2' or self.mlp_act_type != 'gelu_2':
            raise ValueError('RankMixer v6-E2 requires GELU2 projections and task head')
        if abs(self.rm_down_init_scale - 0.01) > 1e-12:
            raise ValueError('RankMixer v6-E2 requires rm_down_init_scale=0.01')
        ...
        if self.rm_dense_trainable_param_count != self.rm_expected_dense_trainable_params:
            raise ValueError(
                'RankMixer v6-E2 dense trainable parameter count={}, expected={}'.format(
    ...
    def _validate_semantic_feature_groups(self):
        ...
        for bucket_name in self._BUCKET_NAMES:
            ...
            if actual_group_sizes != expected_group_sizes:
                raise ValueError(
                    'RankMixer v6-E2 group sizes mismatch for {}: actual={}, expected={}'.format(
                        bucket_name,
                        actual_group_sizes,
            ...
            if checksum != expected_checksum:
                raise ValueError(
                    'RankMixer v6-E2 group checksum mismatch for {}: actual={}, expected={}'.format(
                        bucket_name,
                        checksum,
                        expected_checksum,
                    )
            ...
            logging.info(
                'RankMixer v6-E2 frozen groups %s: version=%s, checksum=%s, groups=%s',
                bucket_name,
                self._GROUP_VERSION,
                checksum,
    ...
    def _verify_graph_dense_trainable_params(self, dense_scope):
        ...
        for variable in dense_variables:
            ...
            if any(dimension is None for dimension in shape):
                raise ValueError(
                    'RankMixer v6-E2 dense variable has unknown shape: {} {}'.format(
                        variable.op.name,
                        shape,
                    )
                )
            ...
            if log_manifest:
                logging.info(
                    'RankMixer v6-E2 dense variable: name=%s shape=%s params=%d',
                    variable.op.name,
                    shape,
                    variable_total,
                )
        ...
        if actual_total != self.rm_expected_dense_trainable_params:
            raise ValueError(
                'RankMixer v6-E2 graph dense trainable parameters={}, expected={}'.format(
                    actual_total,
                    self.rm_expected_dense_trainable_params,
                )
            )
    ...
    def _semantic_tokenize(self, bucket_field_maps, export):
        ...
        with tf.variable_scope(
            ...
            if any(token is None for token in tokens):
                raise ValueError('RankMixer v6-E2 failed to project every local token')
        ...
        for token_index, (bucket_name, group_name, feature_ids, _, input_dim) in enumerate(token_specs):
            logging.info(
                'RankMixer v6-E2 local token %d: bucket=%s, name=%s, fields=%d, input_dim=%d, D=%d',
                token_index,
                bucket_name,
                group_name,
                len(feature_ids),
    ...
    def _build_global_token(self, bucket_tensors, export):
        ...
        logging.info(
            'RankMixer v6-E2 global token: input_dim=%d -> %d -> %d',
            input_dim,
            self.rm_hidden_dim,
            self.rm_hidden_dim,
        )
    ...
    def _rm_mix_tokens(self, inputs):
        ...
        if token_num * head_dim != self.rm_hidden_dim:
            raise ValueError('RankMixer v6-E2 requires H=T so mixed dimension remains D')
        return mixed
    ...
    def _rm_block(self, inputs, block_idx, export):
        ...
        logging.info(
            'RankMixer v6-E2 block %d: input=%s, mixed=%s, reverted=%s, output=%s',
            block_idx,
            inputs.get_shape(),
            mixed.get_shape(),
            reverted.get_shape(),
    ...
    def model_fn(self, features, labels, timestamps=None, mode="train", export=False):
        ...
        with tf.variable_scope(
            ...
            for name in self._BUCKET_NAMES:
                ...
                if any(dim != self.embedding_size for dim in field_dims):
                    raise ValueError(
                        'RankMixer v6-E2 expects every {} field embedding dim to equal {}, got {}'.format(
                            name,
                            self.embedding_size,
        ...
        logging.info(
            "RankMixer v6-E2 output: input_tokens=%s, hidden_tokens=%s, final_tokens=%s, "
