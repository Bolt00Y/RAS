...
This is the production-callable implementation of the frozen D=384, L=3 design:
...
class MLPModel(ModelBase):
    """Base-three-bucket, pure mature RankMixer D=384, L=3 model."""

    _EXPECTED_FIELD_COUNTS = (385, 835, 14)
    _EXPECTED_DENSE_TRAINABLE_PARAMS = 205157727
    ...
    def __init__(self, **_kwargs):
        ...
        logging.info(
            'Pure mature RankMixer D384 L3: fields=%s, tokens=31+1, T=%d, D=%d, '
    ...
    def _validate_architecture_args(self, kwargs):
        ...
        if not self.enable_phalanx:
            raise ValueError('the frozen D384 L3 design requires enable_phalanx=true')
        ...
        if self.enable_dense_warmup:
            raise ValueError('the frozen D384 L3 design requires dense cold start')
    ...
    def _verify_graph_dense_trainable_params(self, dense_scope):
        ...
        for variable in dense_variables:
            ...
            if log_manifest:
                logging.info('D384 L3 dense variable: %s shape=%s params=%d',
    ...
    def model_fn(self, features, labels, timestamps=None, mode='train', export=False):
        ...
        is_train = (mode == 'train')
        logging.info(
            'build Pure Mature RankMixer D384 L3: mode=%s, is_train=%s, partitions=%s',
