...
The default configuration is the production-callable D=384, L=2 design:
...
class MLPModel(ModelBase):
    ...
    _COMPAT_BUILD = 'mature_rankmixer_v3_tf_only_20260901'
    ...
    def __init__(self, **_kwargs):
        ...
        # Configurable architecture.  Defaults reproduce the v3 D384/L2 run.
    ...
    def _validate_feature_contract(self):
        ...
        if self.feature_field_counts != self._EXPECTED_FIELD_COUNTS:
            logging.warning(
                'feature counts differ from the v3 baseline: baseline=%s, actual=%s',
        ...
        for group_name, feature_ids, _ in ordered_groups:
            ...
            if checksum != self._GROUP_CHECKSUMS[group_name]:
                logging.warning(
                    '%s routing differs from the v3 baseline checksum: %s',
                    group_name,
                    checksum,
                )
        ...
        if creative_checksum != self._GROUP_CHECKSUMS['creative']:
            logging.warning(
                'creative routing differs from the v3 baseline checksum: %s',
                creative_checksum,
            )
        ...
        if len(all_ids) != baseline_total:
            logging.warning(
                'routed field count differs from the v3 baseline: baseline=%d, actual=%d',
                baseline_total,
                len(all_ids),
            )
        ...
        if all_checksum != self._ALL_GROUPS_CHECKSUM:
            logging.warning(
                'complete routing differs from the v3 baseline checksum: %s',
                all_checksum,
            )
