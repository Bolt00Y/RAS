...
This is the production-callable implementation of the frozen D=384, L=2 design:
...
* Two mature mix_up + per-token SwiGLU blocks, expansion=3.5 (M=1344).
...
class MLPModel(ModelBase):
    """Base-three-bucket, pure mature RankMixer D=384, L=2 model."""

    _EXPECTED_FIELD_COUNTS = (385, 835, 14)
    _EXPECTED_DENSE_TRAINABLE_PARAMS = 155511711
    _GROUP_VERSION = 'pure_mature_rankmixer_base_3bucket_d384_l2_v3'
    _GROUP_CHECKSUMS = {
    ...
    def __init__(self, **_kwargs):
        ...
        self.mlp_mixer_layers = int(_kwargs.get('mlp_mixer_layers', 2))
        ...
        logging.info(
            'Pure mature RankMixer D384 L2: fields=%s, tokens=31+1, T=%d, D=%d, '
    ...
    def _validate_architecture_args(self, kwargs):
        required = {
            ...
            'mixup_token_dim': (self.mixup_token_dim, 384),
            'mlp_mixer_layers': (self.mlp_mixer_layers, 2),
            'mixer_hidden_dim': (self.mixer_hidden_dim, 1344),
            'global_token_hidden_dim': (self.global_token_hidden_dim, 512),
            'creative_output_dim': (self.creative_output_dim, 48),
        ...
        if not self.enable_phalanx:
            raise ValueError('the frozen D384 L2 design requires enable_phalanx=true')
        ...
        if self.enable_dense_warmup:
            raise ValueError('the frozen D384 L2 design requires dense cold start')
    ...
    def _verify_graph_dense_trainable_params(self, dense_scope):
        ...
        for variable in dense_variables:
            ...
            if log_manifest:
                logging.info('D384 L2 dense variable: %s shape=%s params=%d',
    ...
    def model_fn(self, features, labels, timestamps=None, mode='train', export=False):
        ...
        is_train = (mode == 'train')
        logging.info(
            'build Pure Mature RankMixer D384 L2: mode=%s, is_train=%s, partitions=%s',
        ...
        with tf.variable_scope(
            ...
            if context.shape[1].value != 432:
                raise ValueError('task context must be 384+48=432, got {}'.format(
