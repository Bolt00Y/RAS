# -*- coding: utf-8 -*-
# rankmixer_v4版本：完整实现训练生命周期，按硬编码业务语义分组生成common/item/creative token
import os
import math
...
class MLPModel(ModelBase):
    ...
    _QUERY_TOKEN_NAME = 'common_query_intent_retrieval'
    _QI_TARGET_TOKEN_NAMES = (
        'item_text_relevance',
    ...
    def __init__(self, **_kwargs):
        ...
        self.rm_use_query_item_cross = bool(_kwargs.get('rm_use_query_item_cross', False))
        self.rm_qi_cross_dim = int(_kwargs.get('rm_qi_cross_dim', 128))
        self.rm_qi_cross_targets = _kwargs.get(
            'rm_qi_cross_targets',
            list(self._QI_TARGET_TOKEN_NAMES),
        )
        self.rm_qi_cross_gate_init = float(_kwargs.get('rm_qi_cross_gate_init', -2.0))

        if not isinstance(self.rm_qi_cross_targets, (list, tuple)):
            raise ValueError('rm_qi_cross_targets must be a list or tuple')
        self.rm_qi_cross_targets = list(self.rm_qi_cross_targets)
        if self.rm_qi_cross_dim <= 0 or self.rm_qi_cross_dim > self.rm_hidden_dim:
            raise ValueError(
                'rm_qi_cross_dim={} must be in (0, {}]'.format(
                    self.rm_qi_cross_dim,
                    self.rm_hidden_dim,
                )
            )
        if len(set(self.rm_qi_cross_targets)) != len(self.rm_qi_cross_targets):
            raise ValueError('rm_qi_cross_targets contains duplicated target names')
        unknown_qi_targets = set(self.rm_qi_cross_targets) - set(self._QI_TARGET_TOKEN_NAMES)
        if unknown_qi_targets:
            raise ValueError(
                'unsupported rm_qi_cross_targets: {}'.format(sorted(unknown_qi_targets))
            )
        if self.rm_use_query_item_cross and not self.rm_qi_cross_targets:
            raise ValueError('rm_qi_cross_targets cannot be empty when QICross is enabled')
        ...
        if nonempty_unsupported:
            raise ValueError(
                'Semantic-Cross RankMixer v4 accepts only common/item/creative; '
        ...
        self.rm_semantic_token_index = self._build_semantic_token_index()
        required_qi_tokens = [self._QUERY_TOKEN_NAME] + self.rm_qi_cross_targets
        missing_qi_tokens = [
            name for name in required_qi_tokens if name not in self.rm_semantic_token_index
        ]
        if missing_qi_tokens:
            raise ValueError(
                'QICross semantic tokens are missing: {}'.format(missing_qi_tokens)
            )
        ...
        logging.info(
            'Semantic-Cross RankMixer v4: fields=%s, bucket_tokens=%s, T=%d, H=%d, '
            'D=%d, L=%d, k=%d, token_proj_act=%s, senet=%s, gated_pool=%s, '
            'bucket_cross=%s, query_item_cross=%s, qi_dim=%d, qi_targets=%s',
            field_counts,
            ...
            self.rm_use_query_item_cross,
            self.rm_qi_cross_dim,
            self.rm_qi_cross_targets,
        )
    ...
    def _validate_semantic_feature_groups(self):
        ...
        for bucket_name in self._BUCKET_NAMES:
            ...
            logging.info(
                'RankMixer v4 semantic groups %s: %s',
                bucket_name,
                [(name, len(ids)) for name, ids in bucket_groups],
            )

    def _build_semantic_token_index(self):
        """Build stable global token indices from semantic group names."""
        token_index = {}
        global_index = 0
        for bucket_name in self._BUCKET_NAMES:
            for group_name, _ in self.rm_semantic_feature_groups[bucket_name]:
                if group_name in token_index:
                    raise ValueError('duplicated semantic token name: {}'.format(group_name))
                token_index[group_name] = global_index
                global_index += 1

        if global_index != self.rm_token_num:
            raise ValueError(
                'semantic token index size={} must equal rm_token_num={}'.format(
                    global_index,
                    self.rm_token_num,
                )
            )
    ...
    def build_summary(self, mode):
        ...
        summaries = [loss_summary, auc_summary, copc_summary]

        if self.rm_use_query_item_cross:
            for target_name in self.rm_qi_cross_targets:
                summaries.append(
                    tf.summary.scalar(
                        '{}/qi/{}/gate_mean'.format(mode, target_name),
                        self.qi_gate_mean_tensors[target_name],
                    )
                )
            summaries.extend([
                tf.summary.scalar(
                    '{}/qi/residual_norm'.format(mode),
                    self.qi_residual_norm,
                ),
                tf.summary.scalar(
                    '{}/qi/pooled_context_norm'.format(mode),
                    self.qi_pooled_context_norm,
                ),
                tf.summary.scalar(
                    '{}/qi/residual_to_pool_ratio'.format(mode),
                    self.qi_residual_to_pool_ratio,
                ),
            ])
    ...
    def _query_item_cross_residual(self, input_tokens, export):
        """Directed low-rank query-to-item cross on semantic input tokens."""
        token_dim = input_tokens.shape[-1].value
        if token_dim != self.rm_hidden_dim:
            raise ValueError(
                'QICross input dim={} must equal rm_hidden_dim={}'.format(
                    token_dim,
                    self.rm_hidden_dim,
                )
            )

        query_index = self.rm_semantic_token_index[self._QUERY_TOKEN_NAME]
        # Read pre-Mixer tokens so query/item positions still carry pure semantic meanings.
        query_token = input_tokens[:, query_index, :]
        residuals = []
        gate_means = {}
        ...
        with tf.variable_scope(
            ...
            query_token = layer_norm(query_token, name='query_ln', export=export)
            query_low = tf.contrib.layers.fully_connected(
                inputs=query_token,
                num_outputs=self.rm_qi_cross_dim,
                activation_fn=tf.identity,
            ...
            for target_name in self.rm_qi_cross_targets:
                target_index = self.rm_semantic_token_index[target_name]
                target_token = input_tokens[:, target_index, :]

                with tf.variable_scope(
                    'q_to_{}'.format(target_name),
                    reuse=tf.AUTO_REUSE,
                    partitioner=self.partitioner,
                ):
                    target_token = layer_norm(target_token, name='item_ln', export=export)
                    target_low = tf.contrib.layers.fully_connected(
                        inputs=target_token,
                        num_outputs=self.rm_qi_cross_dim,
                        activation_fn=tf.identity,
                        weights_initializer=self.get_init(self.rm_hidden_dim),
                        weights_regularizer=tf.contrib.layers.l2_regularizer(self.l2_deep),
                        biases_initializer=tf.zeros_initializer(),
                        scope='item_projection',
                    )

                    product = query_low * target_low
                    pair_input = tf.concat(
                        [query_low, target_low, product, query_low - target_low],
                        axis=-1,
                    )
                    pair_hidden = tf.contrib.layers.fully_connected(
                        inputs=pair_input,
                        num_outputs=self.rm_qi_cross_dim,
                        activation_fn=self.get_act_func(self.rm_ffn_act),
                        weights_initializer=self.get_init(4 * self.rm_qi_cross_dim),
                        weights_regularizer=tf.contrib.layers.l2_regularizer(self.l2_deep),
                        biases_initializer=tf.zeros_initializer(),
                        scope='pair_hidden',
                    )

                    gate_input = tf.concat([query_low, target_low, product], axis=-1)
                    gate_logit = tf.contrib.layers.fully_connected(
                        inputs=gate_input,
                        num_outputs=1,
                        activation_fn=tf.identity,
                        weights_initializer=tf.zeros_initializer(),
                        biases_initializer=tf.constant_initializer(self.rm_qi_cross_gate_init),
                        scope='gate',
                    )
                    gate = tf.nn.sigmoid(gate_logit)

                    # Zero output projection keeps the restored v3 prediction unchanged at step 0.
                    projected_residual = tf.contrib.layers.fully_connected(
                        inputs=pair_hidden,
                        num_outputs=self.rm_hidden_dim,
                        activation_fn=tf.identity,
                        weights_initializer=tf.zeros_initializer(),
                    ...
                    residuals.append(gate * projected_residual)
                    gate_means[target_name] = tf.reduce_mean(gate)

        qi_residual = residuals[0] if len(residuals) == 1 else tf.add_n(residuals)
        logging.info(
            'QICross-Lite: query=%s, targets=%s, low_dim=%d, output=%s',
            self._QUERY_TOKEN_NAME,
            self.rm_qi_cross_targets,
            self.rm_qi_cross_dim,
            qi_residual.get_shape(),
        )
        return qi_residual, gate_means
    ...
    def model_fn(self, features, labels, timestamps=None, mode="train", export=False):
        ...
        with tf.variable_scope("Cvr-task-part", reuse=tf.AUTO_REUSE, partitioner=self.partitioner):
            ...
            pooled_context = self._pool_tokens(hidden_tokens)
            residuals = []
            ...
            if self.rm_use_query_item_cross:
                qi_residual, qi_gate_means = self._query_item_cross_residual(
                    input_tokens,
                    export,
                )
                residuals.append(qi_residual)
                self.qi_gate_mean_tensors = qi_gate_means
                self.qi_residual_norm = tf.reduce_mean(
                    tf.sqrt(tf.reduce_sum(tf.square(qi_residual), axis=-1) + 1e-8)
                )
                self.qi_pooled_context_norm = tf.reduce_mean(
                    tf.sqrt(tf.reduce_sum(tf.square(pooled_context), axis=-1) + 1e-8)
                )
                self.qi_residual_to_pool_ratio = (
                    self.qi_residual_norm / (self.qi_pooled_context_norm + 1e-8)
                )

            context = pooled_context
            if residuals:
                total_residual = residuals[0] if len(residuals) == 1 else tf.add_n(residuals)
                with tf.variable_scope("rm_fusion_norm", reuse=tf.AUTO_REUSE):
                    context = layer_norm(context + total_residual, name="ln", export=export)
