# -*- coding: utf-8 -*-
# RankMixer v6-E2-Small：v6-E2 的 D=256 独立版本，SwiGLU M=704 保持不变。
# 1. 输入塔保持 Base/v6 的三桶 BN、Hierarchical SENet 和冻结的 31 组语义字段映射。
# 2. 31 个 Local Token 与 1 个 Global Token 均投影为 D=256；投影后、两个 RankMixer
...
# 5. Dense 可训练参数量为 102,356,069；该数值仅用于防止实现漂移，不作为实验约束；
...
# 7. D=512→256 会改变模型容量，不能保证与原版的预测/AUC 相同。
#    等价性仅针对相同 D=256、相同参数与输入的优化/参考路径。
...
class MLPModel(ModelBase):
    ...
    _EXPECTED_DENSE_TRAINABLE_PARAMS = 102356069
    ...
    def __init__(self, **_kwargs):
        ...
        # RankMixer v6-E2-Small: 31 local tokens + 1 global token, H=T=32, D=256, L=2.
        ...
        if not self.use_rankmixer:
            raise ValueError('RankMixer v6-E2-Small requires use_rankmixer=true')
        if self.embedding_size != 17:
            raise ValueError('RankMixer v6-E2-Small requires embedding_size=17, got {}'.format(self.embedding_size))
        ...
        if self.rm_head_num != self.rm_token_num:
            raise ValueError('RankMixer v6-E2-Small requires rm_head_num == rm_token_num')
        ...
        if self.rm_norm_type != 'rms_norm':
            raise ValueError(
                'RankMixer v6-E2-Small requires rm_norm_type=rms_norm, got {}'.format(
                    self.rm_norm_type
                )
            )
        if self.rm_rms_epsilon <= 0.0:
            raise ValueError('RankMixer v6-E2-Small requires rm_rms_epsilon > 0')
        if self.rm_readout_type != 'pure_flat':
            raise ValueError('RankMixer v6-E2-Small requires rm_readout_type=pure_flat')
        ...
        if nonempty_unsupported:
            raise ValueError(
                'RankMixer v6-E2-Small accepts only common/item/creative; '
        ...
        for arg_name, (actual_value, required_value) in required_architecture.items():
            if actual_value != required_value:
                raise ValueError(
                    'RankMixer v6-E2-Small requires {}={}, got {}'.format(
                        arg_name, required_value, actual_value
                    )
                )
        if tuple(field_counts) != self._EXPECTED_FIELD_COUNTS:
            raise ValueError(
                'RankMixer v6-E2-Small requires field counts={}, got {}'.format(
                    self._EXPECTED_FIELD_COUNTS,
                    tuple(field_counts),
                )
            )
        if self.cvr_layers != [2048, 2048, 256]:
            raise ValueError('RankMixer v6-E2-Small requires cvr_layers=[2048, 2048, 256]')
        if not self.use_senet or not self.use_senet_bn or not self.batch_norm:
            raise ValueError(
                'RankMixer v6-E2-Small requires use_senet/use_senet_bn/batch_norm all true'
            )
        if self.use_mlp_gate:
            raise ValueError('RankMixer v6-E2-Small requires use_mlp_gate=false for a single task path')
        if self.rm_token_proj_act != 'gelu_2' or self.mlp_act_type != 'gelu_2':
            raise ValueError('RankMixer v6-E2-Small requires GELU2 projections and task head')
        if abs(self.rm_down_init_scale - 0.01) > 1e-12:
            raise ValueError('RankMixer v6-E2-Small requires rm_down_init_scale=0.01')
        ...
        if self.rm_dense_trainable_param_count != self.rm_expected_dense_trainable_params:
            raise ValueError(
                'RankMixer v6-E2-Small dense trainable parameter count={}, expected={}'.format(
                    self.rm_dense_trainable_param_count,
                    self.rm_expected_dense_trainable_params,
                )
            )
        logging.info(
            'RankMixer v6-E2-Small: group_version=%s, fields=%s, local_bucket_tokens=%s, '
        ...
        logging.info(
            'RankMixer v6-E2-Small tokenization: optimized=%s; '
    ...
    def _validate_semantic_feature_groups(self):
        ...
        for bucket_name in self._BUCKET_NAMES:
            ...
            if actual_group_sizes != expected_group_sizes:
                raise ValueError(
                    'RankMixer v6-E2-Small group sizes mismatch for {}: actual={}, expected={}'.format(
                        bucket_name,
                        actual_group_sizes,
            ...
            if checksum != expected_checksum:
                raise ValueError(
                    'RankMixer v6-E2-Small group checksum mismatch for {}: actual={}, expected={}'.format(
                        bucket_name,
                        checksum,
                        expected_checksum,
                    )
            ...
            logging.info(
                'RankMixer v6-E2-Small frozen groups %s: version=%s, checksum=%s, groups=%s',
                bucket_name,
                self._GROUP_VERSION,
                checksum,
    ...
    def _verify_graph_dense_trainable_params(self, dense_scope):
        ...
        for variable in dense_variables:
            ...
            if any(dimension is None for dimension in shape):
                raise ValueError(
                    'RankMixer v6-E2-Small dense variable has unknown shape: {} {}'.format(
                        variable.op.name,
                        shape,
                    )
                )
            ...
            if log_manifest:
                logging.info(
                    'RankMixer v6-E2-Small dense variable: name=%s shape=%s params=%d',
                    variable.op.name,
                    shape,
                    variable_total,
                )
        ...
        if actual_total != self.rm_expected_dense_trainable_params:
            raise ValueError(
                'RankMixer v6-E2-Small graph dense trainable parameters={}, expected={}'.format(
                    actual_total,
                    self.rm_expected_dense_trainable_params,
                )
            )
    ...
    def _semantic_tokenize(self, bucket_field_maps, export):
        ...
        with tf.variable_scope(
            ...
            if any(token is None for token in tokens):
                raise ValueError('RankMixer v6-E2-Small failed to project every local token')
        ...
        for token_index, (bucket_name, group_name, feature_ids, _, input_dim) in enumerate(token_specs):
            logging.info(
                'RankMixer v6-E2-Small local token %d: bucket=%s, name=%s, fields=%d, input_dim=%d, D=%d',
                token_index,
                bucket_name,
                group_name,
                len(feature_ids),
    ...
    def _build_global_token(self, bucket_tensors, export):
        ...
        logging.info(
            'RankMixer v6-E2-Small global token: input_dim=%d -> %d -> %d',
            input_dim,
            self.rm_hidden_dim,
            self.rm_hidden_dim,
        )
    ...
    def _rm_mix_tokens(self, inputs):
        ...
        if token_num * head_dim != self.rm_hidden_dim:
            raise ValueError('RankMixer v6-E2-Small requires H=T so mixed dimension remains D')
        return mixed
    ...
    def _rm_block(self, inputs, block_idx, export):
        ...
        logging.info(
            'RankMixer v6-E2-Small block %d: input=%s, mixed=%s, reverted=%s, output=%s',
            block_idx,
            inputs.get_shape(),
            mixed.get_shape(),
            reverted.get_shape(),
    ...
    def model_fn(self, features, labels, timestamps=None, mode="train", export=False):
        ...
        with tf.variable_scope(
            ...
            for name in self._BUCKET_NAMES:
                ...
                if any(dim != self.embedding_size for dim in field_dims):
                    raise ValueError(
                        'RankMixer v6-E2-Small expects every {} field embedding dim to equal {}, got {}'.format(
                            name,
                            self.embedding_size,
        ...
        logging.info(
            "RankMixer v6-E2-Small output: input_tokens=%s, hidden_tokens=%s, final_tokens=%s, "
