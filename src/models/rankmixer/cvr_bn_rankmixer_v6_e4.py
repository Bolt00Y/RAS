# -*- coding: utf-8 -*-
# RankMixer v6-E4：由 cvr_bn_rankmixer_v6_e2_small.py 发展而来的独立版本。
# 1. 保留 v6-E2-Small 的三桶输入 BN、字段级 SENet、D=256、H=T=32、L=2，
#    以及 M=704 的双 Per-token SwiGLU、Mixing/Reverting、长残差和初始化方式。
# 2. SENet 路由为 common→common、common+item→item、creative→creative；
#    隐层仍为 128，creative 只进入独立旁路。
# 3. 使用 cvr_senet_mature_rankmixer_v5 的 10 个 common + 21 个 item 语义分组；
#    Local 投影保持 Small 的 GELU2/RMSNorm 和同宽分组批量 MatMul/Unpack 优化。
# 4. Global 仅取 SENet 前、入口 BN 后的 common+item：[B,20740]→256→256；
#    31 Local + 1 Global 一起进入 [B,32,256] 的 RankMixer 主干。
# 5. 读出参考 cvr_senet_mixer_new_rk_distill_seq_aux_v5_replay.py：
#    主干末端使用共享 gamma/beta [256] 的 LayerNorm，再对 32 个 Token 取均值；
#    creative 使用 238→256→32 的两层 Dense/BN/参数化 Swish，随后直接拼接。
# 6. [B,256]+[B,32]→[B,288]→256→128→1；拼接前不增加额外分支归一化。
#    Block 内仍使用 RMSNorm；最终 LayerNorm epsilon=1e-8，Swish beta 初值为 1.702。
# 7. Dense 可训练参数量为 80,739,301，不含稀疏 Embedding、优化器状态、指标变量
#    和 BN moving statistics。本版本是结构实验，不与 v6-E2-Small 做预测等价承诺。
# 8. 保持原 Python 3 / TensorFlow 1.x / Flood 接口和训练、评估、导出流程；
#    不导入、不继承其他 RankMixer 实现，不增加 phalanx/cayman 或自定义算子依赖。
import os
import math
import hashlib
...
class MLPModel(ModelBase):
    ...
    _LOCAL_BUCKET_NAMES = ('common', 'item')
    _EXPECTED_FIELD_COUNTS = (385, 835, 14)
    _EXPECTED_DENSE_TRAINABLE_PARAMS = 80739301
    _GROUP_VERSION = 'rankmixer_v6_e4_common_item_semantic_v1'
    _GROUP_CHECKSUMS = {
        'common': '61602847a993a6103b9c21b4d6ff2d1817a848d8717e7b201eea4be6fc29bda3',
        'item': 'bc60718450d1b06c55ee60c6a7eb69599722fddd16583f1c1c219f1340f060d3',
        'creative': '956056a173d6daa8b62602b62bf9bd83c638e362c6824aa9cd2ef1300490d10c',
    }
    _GROUP_SIZES = {
        'common': (39, 39, 39, 39, 39, 38, 38, 38, 38, 38),
        'item': (42, 42, 42, 42, 42, 35, 35, 35, 35, 35, 35,
    ...
    def __init__(self, **_kwargs):
        ...
        # Retain Small's field-wise SENet; creative has its own conditioning input.
        ...
        # RankMixer v6-E4: 31 local tokens + 1 global token, H=T=32, D=256, L=2.
        ...
        self.rm_readout_type = _kwargs.get('rm_readout_type', 'mean_pool_creative')
        ...
        if not self.use_rankmixer:
            raise ValueError('RankMixer v6-E4 requires use_rankmixer=true')
        if self.embedding_size != 17:
            raise ValueError('RankMixer v6-E4 requires embedding_size=17, got {}'.format(self.embedding_size))
        ...
        if self.rm_head_num != self.rm_token_num:
            raise ValueError('RankMixer v6-E4 requires rm_head_num == rm_token_num')
        ...
        if self.rm_norm_type != 'rms_norm':
            raise ValueError(
                'RankMixer v6-E4 requires rm_norm_type=rms_norm, got {}'.format(
                    self.rm_norm_type
                )
            )
        if self.rm_rms_epsilon <= 0.0:
            raise ValueError('RankMixer v6-E4 requires rm_rms_epsilon > 0')
        if self.rm_final_norm_type != 'layer_norm':
            raise ValueError('RankMixer v6-E4 requires rm_final_norm_type=layer_norm')
        if self.rm_final_ln_epsilon <= 0.0:
            raise ValueError('RankMixer v6-E4 requires rm_final_ln_epsilon > 0')
        if self.rm_readout_type != 'mean_pool_creative':
            raise ValueError('RankMixer v6-E4 requires rm_readout_type=mean_pool_creative')
        ...
        if nonempty_unsupported:
            raise ValueError(
                'RankMixer v6-E4 accepts only common/item/creative; '
        ...
        semantic_bucket_token_counts = [
            len(self.rm_semantic_feature_groups[name]) if name in self._LOCAL_BUCKET_NAMES else 0
        ...
        for arg_name, (actual_value, required_value) in required_architecture.items():
            if actual_value != required_value:
                raise ValueError(
                    'RankMixer v6-E4 requires {}={}, got {}'.format(
                        arg_name, required_value, actual_value
                    )
                )
        if tuple(field_counts) != self._EXPECTED_FIELD_COUNTS:
            raise ValueError(
                'RankMixer v6-E4 requires field counts={}, got {}'.format(
                    self._EXPECTED_FIELD_COUNTS,
                    tuple(field_counts),
                )
            )
        if self.cvr_layers != [256, 128]:
            raise ValueError('RankMixer v6-E4 requires cvr_layers=[256, 128]')
        if not self.use_senet or not self.use_senet_bn or not self.batch_norm:
            raise ValueError(
                'RankMixer v6-E4 requires use_senet/use_senet_bn/batch_norm all true'
            )
        if self.use_mlp_gate:
            raise ValueError('RankMixer v6-E4 requires use_mlp_gate=false for a single task path')
        if self.rm_token_proj_act != 'gelu_2' or self.mlp_act_type != 'gelu_2':
            raise ValueError('RankMixer v6-E4 requires GELU2 projections and task head')
        if abs(self.rm_down_init_scale - 0.01) > 1e-12:
            raise ValueError('RankMixer v6-E4 requires rm_down_init_scale=0.01')
        ...
        if self.rm_dense_trainable_param_count != self.rm_expected_dense_trainable_params:
            raise ValueError(
                'RankMixer v6-E4 dense trainable parameter count={}, expected={}'.format(
                    self.rm_dense_trainable_param_count,
                    self.rm_expected_dense_trainable_params,
                )
            )
        logging.info(
            'RankMixer v6-E4: group_version=%s, fields=%s, local_bucket_tokens=%s, '
            'T=%d, H=%d, D=%d, L=%d, swiglu_hidden=%d, down_init_scale=%s, '
            'token_proj_act=%s, senet=%s, norm=%s, readout=%s',
            self._GROUP_VERSION,
            field_counts,
        ...
        logging.info(
            'RankMixer v6-E4 tokenization: optimized=%s; '
            'same projection/layout/activation; eliminate repeated slice-gradient buffers',
            self.rm_optimize_tokenize,
        )
        logging.info(
            'RankMixer v6-E4 readout: final_norm=%s, mean_pool=%d, creative=%d->%d, '
            'context=%d, head=%s; derived from v6-E2-Small',
            self.rm_final_norm_type,
            self.rm_hidden_dim,
            self.creative_hidden_dim,
            self.creative_output_dim,
            self.rm_hidden_dim + self.creative_output_dim,
            self.cvr_layers,
        )
    ...
    def _build_semantic_feature_groups():
        """Freeze v5 common/item local groups and the creative-only bypass fields."""
        # Preserve explicit feature IDs; creative is validated here but never tokenized.
        return {
            'common': [
            ...
            'item': [
                ...
                ('item_price_promotion_buypower_context', [
                    '201735', '206550', '302502', '861219', '870008', '870012', '870303', '622533',
                    '10520', '110041', '140700', '500001', '864219', '870310', '600001', '881108',
                    '10216', '10387', '10388', '25027', '5019', '33866926', '33868954', '33868970',
                    '33868978', '870177', '208030', '208034', '208016', '310601', '310602', '310604',
                    '6011', '60119', '6047',
                ]),
                # 商品类目全局漏斗统计（42 个字段）
    ...
    def _validate_semantic_feature_groups(self):
        ...
        for bucket_name in self._BUCKET_NAMES:
            ...
            if actual_group_sizes != expected_group_sizes:
                raise ValueError(
                    'RankMixer v6-E4 group sizes mismatch for {}: actual={}, expected={}'.format(
                        bucket_name,
                        actual_group_sizes,
            ...
            if checksum != expected_checksum:
                raise ValueError(
                    'RankMixer v6-E4 group checksum mismatch for {}: actual={}, expected={}'.format(
                        bucket_name,
                        checksum,
                        expected_checksum,
                    )
            ...
            logging.info(
                'RankMixer v6-E4 frozen groups %s: version=%s, checksum=%s, groups=%s',
                bucket_name,
                self._GROUP_VERSION,
                checksum,
    ...
    def _calculate_dense_trainable_params(self):
        ...
        main_input_dim = (common_fields + item_fields) * self.embedding_size
        creative_input_dim = creative_fields * self.embedding_size
        ...
        senet_params = (
            ...
            + creative_fields * self.senet_hidden_size
        ...
        # Projection kernels/biases are token-specific. RMSNorm follows v6:
        # Local/Block use per-token gamma; Global uses shared gamma.
        # Final LayerNorm shares one [D] gamma/beta pair across all 32 tokens.
        local_token_params = (
            main_input_dim * self.rm_hidden_dim
        ...
        # Each creative Dense has a bias, two BN parameters and a Swish beta.
        creative_params = (
            creative_input_dim * self.creative_hidden_dim
            + self.creative_hidden_dim * self.creative_output_dim
            + 4 * (self.creative_hidden_dim + self.creative_output_dim)
        )

        task_head_params = 0
        task_head_input_dim = self.rm_hidden_dim + self.creative_output_dim
    ...
    def _verify_graph_dense_trainable_params(self, dense_scope):
        ...
        for variable in dense_variables:
            ...
            if any(dimension is None for dimension in shape):
                raise ValueError(
                    'RankMixer v6-E4 dense variable has unknown shape: {} {}'.format(
                        variable.op.name,
                        shape,
                    )
                )
            ...
            if log_manifest:
                logging.info(
                    'RankMixer v6-E4 dense variable: name=%s shape=%s params=%d',
                    variable.op.name,
                    shape,
                    variable_total,
                )
        ...
        if actual_total != self.rm_expected_dense_trainable_params:
            raise ValueError(
                'RankMixer v6-E4 graph dense trainable parameters={}, expected={}'.format(
                    actual_total,
                    self.rm_expected_dense_trainable_params,
                )
            )
    ...
    def senet_layer(self, common_embedding, item_embedding, creative_embedding, is_train, export):
        """Small field-wise gates: common->common, common/item->item, creative->creative."""
        ...
        with tf.variable_scope("senet", reuse=tf.AUTO_REUSE, partitioner=self.partitioner):
            ...
            weight_creative_in = tf.get_variable(
                shape=[creative_field_num, self.senet_hidden_size],
                initializer=tf.glorot_uniform_initializer(),
                name="creative_weight_in",
            )
    ...
    def _semantic_tokenize(self, bucket_field_maps, export):
        """Build the frozen 10 common + 21 item local tokens in canonical order."""
        token_specs = []
        for bucket_name in self._LOCAL_BUCKET_NAMES:
        ...
        with tf.variable_scope(
            ...
            if any(token is None for token in tokens):
                raise ValueError('RankMixer v6-E4 failed to project every local token')
        ...
        for token_index, (bucket_name, group_name, feature_ids, _, input_dim) in enumerate(token_specs):
            logging.info(
                'RankMixer v6-E4 local token %d: bucket=%s, name=%s, fields=%d, input_dim=%d, D=%d',
                token_index,
                bucket_name,
                group_name,
                len(feature_ids),
    ...
    def _build_global_token(self, bucket_tensors, export):
        """Encode pre-SENet, post-input-BN common/item fields into one global token."""
        global_input = tf.concat(
            [bucket_tensors[name] for name in self._LOCAL_BUCKET_NAMES],
            axis=-1,
        )
        ...
        logging.info(
            'RankMixer v6-E4 global token: input_dim=%d -> %d -> %d',
            input_dim,
            self.rm_hidden_dim,
            self.rm_hidden_dim,
        )
    ...
    def _rm_mix_tokens(self, inputs):
        ...
        if token_num * head_dim != self.rm_hidden_dim:
            raise ValueError('RankMixer v6-E4 requires H=T so mixed dimension remains D')
        return mixed
    ...
    def _rm_block(self, inputs, block_idx, export):
        ...
        logging.info(
            'RankMixer v6-E4 block %d: input=%s, mixed=%s, reverted=%s, output=%s',
            block_idx,
            inputs.get_shape(),
            mixed.get_shape(),
            reverted.get_shape(),
    ...
    def _rm_final_layer_norm(self, inputs):
        """Replay-v5 readout: normalize each token over D with shared [D] gamma/beta."""
        shape = inputs.get_shape().as_list()
        if len(shape) != 3 or shape[1:] != [self.rm_token_num, self.rm_hidden_dim]:
            raise ValueError(
                'RankMixer v6-E4 final LayerNorm expects [B,{},{}], got {}'.format(
                    self.rm_token_num, self.rm_hidden_dim, shape
                )
            )
        with tf.variable_scope(
            'rm_final_layer_norm',
            ...
            normalized = (inputs - mean) / tf.sqrt(variance + self.rm_final_ln_epsilon)
            return gamma * normalized + beta

    def _creative_converter(self, inputs, is_train, export):
        """Creative bypass: 238->256->32, each Dense followed by BN and trainable Swish."""
        input_dim = inputs.shape[-1].value
        expected_dim = self._EXPECTED_FIELD_COUNTS[2] * self.embedding_size
        if input_dim != expected_dim:
            raise ValueError(
                'RankMixer v6-E4 creative width={}, expected={}'.format(input_dim, expected_dim)
            )

        def apply_bn(tensor, scope):
        ...
        with tf.variable_scope(
            ...
            hidden = tf.contrib.layers.fully_connected(
                inputs=inputs,
                num_outputs=self.creative_hidden_dim,
                activation_fn=None,
                weights_initializer=tf.glorot_uniform_initializer(),
                biases_initializer=tf.zeros_initializer(),
                scope='hidden_dense',
            )
            hidden = apply_bn(hidden, 'bn_hidden')
            hidden_beta = tf.get_variable(
                'swish_beta_hidden',
                shape=[self.creative_hidden_dim],
            ...
            output = tf.contrib.layers.fully_connected(
                inputs=hidden,
                num_outputs=self.creative_output_dim,
                activation_fn=None,
                weights_initializer=tf.glorot_uniform_initializer(),
                biases_initializer=tf.zeros_initializer(),
                scope='projection',
            )
            output = apply_bn(output, 'bn_output')
            output_beta = tf.get_variable(
                'swish_beta_output',
                shape=[self.creative_output_dim],
                initializer=tf.constant_initializer(1.702),
            )
            return output * tf.nn.sigmoid(output_beta * output)

    def _task_head(self, context, is_train, export):
        """v5-style single-task tower: [256 pooled + 32 creative]->256->128->1."""
        hidden = context
    ...
    def model_fn(self, features, labels, timestamps=None, mode="train", export=False):
        ...
        with tf.variable_scope(
            ...
            for name in self._BUCKET_NAMES:
                ...
                if any(dim != self.embedding_size for dim in field_dims):
                    raise ValueError(
                        'RankMixer v6-E4 expects every {} field embedding dim to equal {}, got {}'.format(
                            name,
                            self.embedding_size,
            ...
            global_token = self._build_global_token(normalized_buckets, export)
            ...
            creative_context = self._creative_converter(
                bucket_tensors['creative'], is_train, export
            )
            # Match replay-v5: concat immediately after pooling and creative Swish.
            context = tf.concat(
                [mixer_context, creative_context],
                axis=1,
                name='rm_plus_creative',
            )
            context_dim = self.rm_hidden_dim + self.creative_output_dim
            context.set_shape([None, context_dim])
        ...
        logging.info(
            "RankMixer v6-E4 output: input_tokens=%s, hidden_tokens=%s, final_tokens=%s, "
            "pooled=%s, creative=%s, context=%s, dense_params=%d, graph_dense_params=%d",
            input_tokens.get_shape(),
            hidden_tokens.get_shape(),
            final_tokens.get_shape(),
            mixer_context.get_shape(),
            creative_context.get_shape(),
            context.get_shape(),
            self.rm_dense_trainable_param_count,
