# -*- coding: utf-8 -*-
# RankMixer v10：v6 语义主干 + 标准 LayerNorm + PureFlat 单路径读出。
...
#    Block 的 PreNorm 位置以及最终输出全部使用标准 LayerNorm。
...
#    [B,32,512] 直接展平为 [B,16384]，进入 [2048,2048,256] 单任务头。
# 5. 在配套 v10 启动配置下，固定 Dense 可训练参数量为 199,275,877；不包含稀疏
#    Embedding 表、优化器状态、指标变量和 BN moving statistics。
...
class MLPModel(ModelBase):
    ...
    _EXPECTED_DENSE_TRAINABLE_PARAMS = 199275877
    _DENSE_TRAINABLE_PARAM_LIMIT = 200000000
    _GROUP_VERSION = 'rankmixer_v10_semantic_balanced_v1'
    _GROUP_CHECKSUMS = {
    ...
    def __init__(self, **_kwargs):
        ...
        # RankMixer v10: 31 local tokens + 1 global token, H=T=32, D=512, L=2.
        ...
        self.rm_norm_type = _kwargs.get('rm_norm_type', 'layer_norm')
        self.rm_readout_type = _kwargs.get('rm_readout_type', 'pure_flat')

        if not self.use_rankmixer:
            raise ValueError('RankMixer v10 requires use_rankmixer=true')
        if self.embedding_size != 17:
            raise ValueError('RankMixer v10 requires embedding_size=17, got {}'.format(self.embedding_size))
        ...
        if self.rm_head_num != self.rm_token_num:
            raise ValueError('RankMixer v10 requires rm_head_num == rm_token_num')
        ...
        if self.rm_norm_type != 'layer_norm':
            raise ValueError('RankMixer v10 requires rm_norm_type=layer_norm')
        if self.rm_readout_type != 'pure_flat':
            raise ValueError('RankMixer v10 requires rm_readout_type=pure_flat')
        ...
        if nonempty_unsupported:
            raise ValueError(
                'RankMixer v10 accepts only common/item/creative; '
        ...
        for arg_name, (actual_value, required_value) in required_architecture.items():
            if actual_value != required_value:
                raise ValueError(
                    'RankMixer v10 requires {}={}, got {}'.format(
                        arg_name, required_value, actual_value
                    )
                )
        if tuple(field_counts) != self._EXPECTED_FIELD_COUNTS:
            raise ValueError(
                'RankMixer v10 requires field counts={}, got {}'.format(
                    self._EXPECTED_FIELD_COUNTS,
                    tuple(field_counts),
                )
            )
        if self.cvr_layers != [2048, 2048, 256]:
            raise ValueError('RankMixer v10 requires cvr_layers=[2048, 2048, 256]')
        if not self.use_senet or not self.use_senet_bn or not self.batch_norm:
            raise ValueError(
                'RankMixer v10 requires use_senet/use_senet_bn/batch_norm all true'
            )
        if self.use_mlp_gate:
            raise ValueError('RankMixer v10 requires use_mlp_gate=false for a single task path')
        if self.rm_token_proj_act != 'gelu_2' or self.mlp_act_type != 'gelu_2':
            raise ValueError('RankMixer v10 requires GELU2 projections and task head')
        if abs(self.rm_down_init_scale - 0.01) > 1e-12:
            raise ValueError('RankMixer v10 requires rm_down_init_scale=0.01')
        ...
        if self.rm_dense_trainable_param_count != self._EXPECTED_DENSE_TRAINABLE_PARAMS:
            raise ValueError(
                'RankMixer v10 dense trainable parameter count={}, expected={}'.format(
        ...
        if self.rm_dense_trainable_param_count >= self._DENSE_TRAINABLE_PARAM_LIMIT:
            raise ValueError(
                'RankMixer v10 dense trainable parameters must remain below {}'.format(
                    self._DENSE_TRAINABLE_PARAM_LIMIT
                )
            )

        logging.info(
            'RankMixer v10: group_version=%s, fields=%s, local_bucket_tokens=%s, '
    ...
    def _validate_semantic_feature_groups(self):
        ...
        for bucket_name in self._BUCKET_NAMES:
            ...
            if actual_group_sizes != expected_group_sizes:
                raise ValueError(
                    'RankMixer v10 group sizes mismatch for {}: actual={}, expected={}'.format(
                        bucket_name,
                        actual_group_sizes,
            ...
            if checksum != expected_checksum:
                raise ValueError(
                    'RankMixer v10 group checksum mismatch for {}: actual={}, expected={}'.format(
                        bucket_name,
                        checksum,
                        expected_checksum,
                    )
            ...
            logging.info(
                'RankMixer v10 frozen groups %s: version=%s, checksum=%s, groups=%s',
                bucket_name,
                self._GROUP_VERSION,
                checksum,
    ...
    def _calculate_dense_trainable_params(self):
        ...
        # Projection kernels/biases are token-specific. Standard LayerNorm keeps
        # one shared gamma/beta pair over D while normalizing every [B,T] row.
        local_token_params = (
        ...
        block_params = self.rm_layer_num * (
            4 * self.rm_hidden_dim
            + 2 * one_swiglu_params
        )
        final_norm_params = 2 * self.rm_hidden_dim

        task_head_params = 0
        task_head_input_dim = self.rm_token_num * self.rm_hidden_dim
    ...
    def _verify_graph_dense_trainable_params(self, dense_scope):
        ...
        for variable in dense_variables:
            ...
            if any(dimension is None for dimension in shape):
                raise ValueError(
                    'RankMixer v10 dense variable has unknown shape: {} {}'.format(
                        variable.op.name,
                        shape,
                    )
                )
            ...
            if log_manifest:
                logging.info(
                    'RankMixer v10 dense variable: name=%s shape=%s params=%d',
                    variable.op.name,
                    shape,
                    variable_total,
                )
        ...
        if actual_total != self._EXPECTED_DENSE_TRAINABLE_PARAMS:
            raise ValueError(
                'RankMixer v10 graph dense trainable parameters={}, expected={}'.format(
                    actual_total,
                    self._EXPECTED_DENSE_TRAINABLE_PARAMS,
                )
            )
    ...
    def _rm_layer_norm(self, inputs, scope, export):
        """Standard LayerNorm over D, independently for every sample/token row."""
        shape = inputs.get_shape().as_list()
        if len(shape) not in (2, 3):
            raise ValueError('LayerNorm expects rank-2 or rank-3 input, got {}'.format(shape))
        hidden_dim = shape[-1]
        if hidden_dim is None:
            raise ValueError('LayerNorm hidden dimension must be statically known')
        return layer_norm(inputs, name=scope, export=export)
    ...
    def _semantic_tokenize(self, bucket_field_maps, export):
        ...
        with tf.variable_scope(
            ...
            if any(token is None for token in tokens):
                raise ValueError('RankMixer v10 failed to project every local token')
            local_tokens = tf.stack(tokens, axis=1)
            local_tokens = self._rm_layer_norm(
                local_tokens,
                scope='local_token_layer_norm',
                export=export,
            )

        for token_index, (bucket_name, group_name, feature_ids, _, input_dim) in enumerate(token_specs):
            logging.info(
                'RankMixer v10 local token %d: bucket=%s, name=%s, fields=%d, input_dim=%d, D=%d',
                token_index,
                bucket_name,
                group_name,
                len(feature_ids),
    ...
    def _build_global_token(self, bucket_tensors, export):
        ...
        with tf.variable_scope(
            ...
            global_token = self._rm_layer_norm(
                global_token,
                scope='layer_norm',
                export=export,
            )
        ...
        logging.info(
            'RankMixer v10 global token: input_dim=%d -> %d -> %d',
            input_dim,
            self.rm_hidden_dim,
            self.rm_hidden_dim,
        )
    ...
    def _rm_mix_tokens(self, inputs):
        ...
        if token_num * head_dim != self.rm_hidden_dim:
            raise ValueError('RankMixer v10 requires H=T so mixed dimension remains D')
        return mixed
    ...
    def _rm_block(self, inputs, block_idx, export):
        """Pre-LayerNorm Mixing/Reverting block with two independent Per-token SwiGLUs."""
        with tf.variable_scope(
            ...
            mixed_norm = self._rm_layer_norm(
                mixed,
                scope='mixed_layer_norm',
                export=export,
            )
            ...
            original_norm = self._rm_layer_norm(
                reverted,
                scope='original_layer_norm',
                export=export,
            )
        ...
        logging.info(
            'RankMixer v10 block %d: input=%s, mixed=%s, reverted=%s, output=%s',
            block_idx,
            inputs.get_shape(),
            mixed.get_shape(),
            reverted.get_shape(),
    ...
    def _task_head(self, context, is_train, export):
        ...
        for layer_idx, layer_size in enumerate(self.cvr_layers):
            ...
            hidden = tf.contrib.layers.fully_connected(
                ...
                weights_initializer=self.get_init(input_dim),
                biases_initializer=tf.zeros_initializer(),
                scope='rm_v10_mlp{}'.format(layer_idx),
            )
            if self.batch_norm:
                hidden = ModelBase.batch_norm_layer_v2(
                    x=hidden,
                    train_phase=is_train,
                    scope_bn='rm_v10_bn_{}'.format(layer_idx),
                    batch_norm_decay=self.batch_norm_decay,
                    use_riemann_bn=self.use_riemann_bn,
                    export=export,
                )
    ...
    def model_fn(self, features, labels, timestamps=None, mode="train", export=False):
        ...
        with tf.variable_scope(
            ...
            for name in self._BUCKET_NAMES:
                ...
                if any(dim != self.embedding_size for dim in field_dims):
                    raise ValueError(
                        'RankMixer v10 expects every {} field embedding dim to equal {}, got {}'.format(
                            name,
                            self.embedding_size,
            ...
            final_tokens = self._rm_layer_norm(
                hidden_tokens,
                scope='rm_final_layer_norm',
                export=export,
            )
        ...
        logging.info(
            "RankMixer v10 output: input_tokens=%s, hidden_tokens=%s, final_tokens=%s, "
