...
Only the architecture is aligned with the mature-v1 algorithm specification.
The Flood dataset lifecycle, optimizer, train/test/export methods and warm-up
contract follow the successfully exercised RankMixer v1-v10 and
``cvr_bn_senet_dcnm_fst`` training runtime.  This is a standalone model module:
it does not import another RankMixer implementation or anything under
``new_rankmixer_0831``.
"""
...
# Keep the schedule-increment UTF-8 workaround from the successful FST shell.
# In the server image os.popen otherwise decodes subprocess output with ASCII.
...
def _utf8_popen(cmd, mode='r', buffering=-1):
    ...
    return os._wrap_close(stream, process)
...
class MLPModel(ModelBase):
    """Frozen D256/L3 mature RankMixer on the proven FST/Flood shell."""

    _RUNTIME_BUILD_ID = 'cvr_mature_rankmixer_fst_v1_20260901'
    ...
    def __init__(self, **_kwargs):
        ...
        if self.runtime_build_id != self._RUNTIME_BUILD_ID:
            raise ValueError(
                'runtime_build_id={} does not match source build {}'.format(
        ...
        logging.info(
            'Mature RankMixer FST v1: build=%s, fields=%s, T=%d, D=%d, '
    ...
    def _validate_architecture_args(self, kwargs):
        ...
        if self.enable_dense_warmup:
            raise ValueError('cvr_mature_rankmixer_fst_v1 requires dense cold start')
        if self.use_mlp_gate:
            raise ValueError('cvr_mature_rankmixer_fst_v1 requires use_mlp_gate=false')
