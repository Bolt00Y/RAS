...
| 工作成果 | 实验或实现依据 | 当前结论 |
|---|---|---|
| 完成从初始 RankMixer 到面向业务语义的输入组织 | v1 裸维度切分 → v2 完整字段切分 → v3 业务语义分组 | 同日 AUC：v3 **0.862991**、v2 **0.862690**，v3 提升 **万3.01**，参数量相同 |
| 完成双空间交互、Global Token 和增强读出的实现 | v5 引入 Mixing/Reverting、双 Per-token SwiGLU、Global/Pool/Flatten 三路读出 | 7 月三个共同测试日，v5 平均 AUC 为 **0.865043**，v3 为 **0.864406**，平均提升 **万6.38** |
| 找到适合继续消融的 v6 起点 | v5 的 D=1024 改为 D=512，同时恢复语义约束的均衡分组 | 8 月首日 AUC：v6 **0.866017**、v5 **0.864163**，提升 **千1.854**；参数从 **348.432M** 降到 **177.217M** |
| 将 v10 代码中的联合改动拆成可解释的验证链 | v6 → E2：增强读出改 PureFlat；E2 → E3：RMSNorm 改 LayerNorm | 三日均值：E2 **0.867497**、v6 **0.867034**；首日 E3 **0.866386**、E2 **0.866562**，E3 回退 **万1.76**；v10 本身尚未运行 |
| 完成宽度和业务分支的进一步取舍 | E2 → E2 Small：D 减半；Small → E4：common/item 主干、creative 旁路及小任务头 | 首日 E2/Small/E4 为 **0.866562/0.866510/0.866333**；Small 比 E2 低 **万0.52**，当前代码参数减少 **48.66%** |
| 建立来自公司线上结构的高质量小模型对照 | mature_v1：D=256、3 层、成熟 SENet/pSwiGLU、均值池化与 creative 旁路 | 两日 mature_v1 为 **0.866934/0.867801**，Base 为 **0.866960/0.867867**，分别低 **万0.26/万0.66** |
...
**实验反馈。**7 月首日，初始未存代码方案为 **0.858606**，BN v1 为 **0.862033**，BN v1 提升 **千3.427**。由于前者代码缺失，这个提升只能归属于两个完整方案之间的差异。BN v1 四个测试日平均 AUC 为 **0.863170**，同日 Base 均值为 **0.865836**，平均落后 **千2.666**，说明后续仍有明显改进空间。
...
**实验反馈。**首日 AUC 从 v1 的 **0.862033** 提升到 v2 的 **0.862690**，增加 **万6.57**；同日 Base 为 **0.864538**，差距由 **千2.505** 缩小到 **千1.848**。参数下降到 **95.809M**。这是整套修正的净效果，无法拆出 SENet、LR reset、字段切分或 Bucket Cross 各自的贡献。仓库中的 `v1_lrfix` 可以作为工程对照实现，但汇总表没有其独立结果。
...
**实验反馈。**同一测试日，v3 AUC 为 **0.862991**，v2 为 **0.862690**，v3 提升 **万3.01**；两者参数同为 **95.809M**。这是本阶段较清楚的语义分组正向证据。v3 在最新表中已有七个测试日，平均 AUC 为 **0.865005**，同日 Base 均值为 **0.866215**，平均落后 **千1.210**；最后三日分别落后 **千1.128 / 千1.137 / 千1.087**。改善后仍存在差距，因此后续继续检查交互能力与读出能力。
...
**实验反馈。**同日 v4 AUC 为 **0.862709**，v3 为 **0.862991**，v4 下降 **万2.82**，并增加约 **0.630M** 参数。这个结果说明当前低秩定向残差分支没有形成净收益。其输入已经经过 token 压缩，交叉范围也只涉及两个目标；后续 v8/v9 转向“压缩之前、完整字段空间中的交叉”，因此具有新的问题依据。
...
**实验反馈与取舍。**v5 在 7 月三个测试日得到 **0.863747 / 0.864929 / 0.866454**，同日 v3 为 **0.862991 / 0.864324 / 0.865902**；v5 分别提升 **万7.56 / 万6.05 / 万5.52**，平均 AUC 为 **0.865043 vs 0.864406**，平均提升 **万6.38**。对上一编号版本 v4，唯一共同测试日的 AUC 为 **0.863747 vs 0.862709**，v5 提升 **千1.038**。v5 相对 Base 的三个同日差值均值为低 **万7.00**。这是该结构组合的改善，不能单独归因为 Global、SwiGLU、深头或某一路读出。
...
**实验反馈。**在同一个 8 月首日测试集上，v6 为 **0.866017**，v5 为 **0.864163**，v6 提升 **千1.854**。这个结果支持“语义分组与宽度控制”组合在该轮更有效，但两项同时改变，无法分别定量归因。

| 测试日 | Base | v6 | v6 − Base |
|---|---:|---:|---:|
| 2026-08-16 | 0.866960 | 0.866017 | −万9.43 |
...
**实验反馈。**8 月首日 v7 AUC 为 **0.865866**，上一编号版本 v6 为 **0.866017**，v7 低 **万1.51**；同日 Base 为 **0.866960**，v7 低 **千1.094**。当前表中没有同一 8 月链的 v3 结果，所以只能评价这个候选的总体位置，**还不能从现有记录给出“深任务头相对 v3 增益多少”的数值**。7 月 v3 与 8 月 v7 的绝对 AUC 差值不能作为该消融的效果。
...
**实验反馈。**8 月首日 v8 AUC 为 **0.866615**，上一编号版本 v7 为 **0.865866**，v8 提升 **万7.49**；其设计父版本 v6 为 **0.866017**，v8 提升 **万5.98**。同日 Base 为 **0.866960**，v8 低 **万3.45**；E2 为 **0.866562**，v8 高 **万0.53**。单日结果支持继续关注这一候选，但无法把“万5.98”的全部差值归因给 Masked DCN。
...
**实验反馈。**8 月首日 v9 AUC 为 **0.865254**，上一版本 v8 为 **0.866615**，v9 低 **千1.361**；同日 Base 为 **0.866960**，v9 低 **千1.706**。这表明“更贴近 Base 的交叉层 + 双视图 + Shortcut”这一组合没有在本次实验中胜出。v9 同时改变交叉形式、Global 来源、Local 投影输入、Flatten 宽度和读出结构，尚不足以判断其中哪一项造成回退。这个结果进一步说明了围绕既有主干进行拆分验证的必要性。
...
**如何评价 v10。**v10 尚未运行，最新 Excel 没有 `bn_rankmixer_v10` 的 AUC。当前代码核对表明，v6_e3 与 v10 使用相同的语义分组、T/D/L/M、全链路 LayerNorm 和 PureFlat 核心结构，但表中的 **0.866386 只记为 E3 的实测结果**，不登记为 v10 结果。上一编号版本 v9 的首日 AUC 为 **0.865254**；由于 v10 没有实测 AUC，不能形成 v10 与 v9 的数值比较。
...
| v10 | 32/512/2 | LayerNorm + PureFlat | — | 未运行；E3 的独立结果见第 5 节 |
...
|---|---:|---:|---:|---:|
| 2026-08-16 | 0.866017 | 0.866562 | **万5.45** | −万3.98 |
| 2026-08-17 | 0.867088 | 0.867488 | **万4.00** | −万3.79 |
| 2026-08-18 | 0.867996 | 0.868440 | **万4.44** | −万4.69 |
| 共同三日均值 | 0.867034 | 0.867497 | **万4.63** | **−万4.15** |
...
**实验反馈。**E3 首日为 **0.866386**，E2 为 **0.866562**，E3 下降 **万1.76**；原始 v6 为 **0.866017**，E3 仍高 **万3.69**。首日变化可以作如下算术分解：
...
**实验反馈。**Small 首日 AUC 为 **0.866510**，E2 为 **0.866562**，Small 低 **万0.52**；同日 Base 为 **0.866960**，Small 低 **万4.50**。这是一个值得保留的成本候选：本次单日结果中的 AUC 损失较小，但多日稳定性与实际训练耗时仍需要后续记录支持。
...
**实验反馈。**E4 参数 **80.739M**，首日 AUC 为 **0.866333**，Small 为 **0.866510**，E4 下降 **万1.77**；同日 Base 为 **0.866960**，E4 低 **万6.27**。结果说明本次多项简化在节省成本的同时牺牲了一部分 AUC；无法单独判定是 creative 路由、均值池化、Final LN 还是小任务头导致回退。
...
| E3 相对 v6 的变化是否来自 LayerNorm？ | E3 0.866386，相比 E2 0.866562 下降万1.76 | 单日具体 Norm 实现对照；v10 尚无实测值 |
| D=512 是否有缩小空间？ | Small 0.866510，相比 E2 0.866562 仅下降万0.52，参数约减半 | 单日成本取舍证据 |
...
|---|---:|---:|---:|---:|
| 2026-08-16 | 0.866960 | **0.866934** | **−万0.26** | 1.005859 |
| 2026-08-17 | 0.867867 | **0.867801** | **−万0.66** | 1.005633 |
...
**实验反馈。**首日 mature_v4 AUC 为 **0.866206**，mature_v1 为 **0.866934**，mature_v4 下降 **万7.28**；同日 Base 为 **0.866960**，mature_v4 低 **万7.54**。当前 v1→v4 联合改变了 token 宽度、FFN 形式和多条分支宽度，因此只能说明这个扩容/简化组合在本次结果中没有胜过 mature_v1，不能把全部下降归因为删除 SwiGLU 的独立门控分支。
...
**实验反馈。**首日 mature_v5 AUC 为 **0.866289**，mature_v4 为 **0.866206**，mature_v5 提升 **万0.83**；相对 mature_v1 的 **0.866934** 仍低 **万6.45**。参数量按当前代码和 args 记为 **135.775M**。v4→v5 保持主干与末端分支不变，但 tokenizer 同时改变语义组织、投影共享范围、激活和参数量，因此结果归属于完整 tokenizer 替换。
...
**实验反馈。**首日 UniMixer v1 AUC 为 **0.865662**，同日 Base 为 **0.866960**，UniMixer v1 低 **千1.298**。该版本同时采用了自己的 token 归一化、SiameseNorm、SwiGLU 和读出，不是仅把 v6 的固定 Mixing 替换成可学习矩阵。因此当前结果能够描述完整 UniMixer 候选的效果，不能单独判断“可学习 Mixing 是否优于固定 Mixing”。
...
所有柱形对应同一个测试日 `2026-08-16`，Base AUC=0.866960。排序只描述这一日的结果，不代表显著性排序；mature_v5 参数量按当前代码口径记录。

| 候选 | 保留理由 | 当前数据范围 |
|---|---|---|
| mature_v1 | 已有结果最接近 Base，且当前实现参数约 110M | 两日 AUC 0.866934/0.867801，分别比 Base 低万0.26/万0.66 |
| v6_e2 | 相对 v6 连续三日正向，可作为自研路线的新对照点 | 三日均值 0.867497 vs 0.867034，提升万4.63 |
| v6_e2_small | 首日 AUC 0.866510，E2 为 0.866562；仅低万0.52，参数约减半 | 目前一日 |
