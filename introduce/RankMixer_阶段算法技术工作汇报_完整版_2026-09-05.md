...
| v6 → v8 | 2026-08-16 | 0.866960 | v6：0.866017 | v8：0.866615 | **万5.98** | −万3.45 |
| E2 → Small | 2026-08-16 | 0.866960 | E2：0.866562 | Small：0.866510 | **−万0.52** | −万4.50 |
| 自研与成熟路线对照 | 08-16～08-17 均值 | 0.867414 | v6：0.866553 | mature_v1：0.867368 | **万8.15** | **−万0.46** |
...
**跨路线训练配置边界。**相同数据与标签不意味着所有保存的训练配置完全相同。当前代码及 args 存在以下区别：

| 当前保存配置 | v6 / E2 / E3 / Small / E4 | mature_v1～v5 |
|---|---:|---:|
| gauss_decay 的 min_rate | 0.1 | 0.5 |
| 解释范围 | v6 消融链在该配置上保持一致 | mature 内部保持一致；跨路线与 BN 系列不同 |

因此，mature 与 BN 系列的结果属于各自整套方案及配置的比较，不能归因于纯结构差异。完整历史日志未保留，上表说明当前保存配置，不据此断言每次历史任务的平台最终取值。
...
**分析。**BN v1 四天均高于初始方案，说明这一轮完整实现取得四个共同测试日的同向改善；但初始方案代码缺失，无法把收益归因到某个具体模块。BN v1 四日仍平均落后 Base 千2.666，因此后续需要优先修复字段切分与输入组织。
...
```mermaid
flowchart TD
    subgraph BEFORE["修改前：BN v1"]
        OLD_A["三桶 BN；裸维度切成 16 Token"] --> OLD_B["2 层 Mixing + PFFN，k=4；每层三次 LN"] --> OLD_C["均值池化 → 线性头"]
    end
    subgraph AFTER["修改后：v2；重点变化见节点"]
...
**公式改动前后。**输入由“展平后按维度切片”改为“按完整字段集合取出”，主干去掉额外的 FFN 前 LayerNorm，读出增加动态汇聚与桶间残差：

| 位置 | 修改前：BN v1 | 修改后：v2 |
|---|---|---|
| Token 输入 | $x_t=\phi(x[a_t:b_t]W_t+b_t)$，切点可落在字段内部 | $x_t=\phi([\widetilde E_j]_{j\in G_t}W_t+b_t)$，保留每字段 17 维 |
| FFN 输入及输出 | $S=\operatorname{LN}_1(X+P(X))$；$X'=\operatorname{LN}_3(S+F(\operatorname{LN}_2(S)))$ | $S=\operatorname{LN}_1(X+P(X))$；$X'=\operatorname{LN}_2(S+F(S))$ |
| 汇聚 | $h=\frac1{16}\sum_t h_t$ | $h=\operatorname{LN}(\sum_t\alpha_t h_t+r)$ |
...
```mermaid
flowchart TD
    subgraph BEFORE["修改前：v2"]
        OLD_A["三桶 BN + SENet → 完整字段连续分组 → 16 Token"] --> OLD_B["固定 Mixing + PFFN → Gated Pool"]
        OLD_A --> OLD_C["输入 Token 的 Bucket Cross"]
        OLD_B --> OLD_D["融合 LN → 线性头"]
        OLD_C --> OLD_D
    end
    subgraph AFTER["修改后：v3；重点变化见节点"]
...
**公式改动前后。**投影形式保持不变，只替换字段集合的定义：

$$
\begin{aligned}
\mathrm{v2}:~x_t&=\phi([\widetilde E_j]_{j\in G_t^{\mathrm{consecutive}}}W_t+b_t),\\
\mathrm{v3}:~x_t&=\phi([\widetilde E_j]_{j\in G_t^{\mathrm{semantic}}}W_t+b_t).
\end{aligned}
$$

前者按桶内顺序连续分段，后者按固定业务语义收集字段 ID；两者 token 数、输出宽度和总投影规模保持一致。新集合满足：
...
```mermaid
flowchart TD
    subgraph BEFORE["修改前：v3"]
        OLD_A["16 个语义输入 Token"] --> OLD_B["原主干 → Gated Pool"]
        OLD_A --> OLD_C["输入 Token 的 Bucket Cross"]
        OLD_B --> OLD_D["融合 LN → 线性头"]
        OLD_C --> OLD_D
    end
    subgraph AFTER["修改后：v4；重点变化见节点"]
...
**融合位置的前后变化。**设 $p$ 为主干池化、$r_B$ 为原 Bucket Cross，则：

$$
h_{\mathrm{v3}}=\operatorname{LN}(p+r_B)
\quad\longrightarrow\quad
h_{\mathrm{v4}}=\operatorname{LN}(p+r_B+r_{QI}).
$$

新增的是并行 Query–Item 残差；原有主干与 Bucket Cross 保留。
...
v5 是一次组合式扩展，参考了 TokenMixer-Large 对 Mixing/Reverting 和残差组织的设计思路；实际实现只采用当前代码中明确存在的 dense 路径，没有把论文中的 MoE、辅助监督或线上收益写成本项目成果。[TokenMixer-Large 论文](https://arxiv.org/abs/2602.06563)
...
| PreNorm、block 输入长残差、小幅度输出初始化 | 为较大主干保留稳定的输入直通路径 |
...
flowchart TD
    subgraph BEFORE["修改前：v3"]
        OLD_A["16 个语义输入 Token；D768"] --> OLD_B["固定 Mixing + PFFN → Gated Pool"]
        OLD_A --> OLD_C["输入 Token 的 Bucket Cross"]
        OLD_B --> OLD_D["融合 LN → 线性头；无 Reverting"]
        OLD_C --> OLD_D
    end
    subgraph AFTER["修改后：v5；重点变化见节点"]
...
| 位置 | 修改前：v3 | 修改后：v5 |
|---|---|---|
| 主干输入 | $X_0=[x_1;\ldots;x_{16}]\in\mathbb R^{16\times768}$ | $X_0=[x_1;\ldots;x_{31};g_0]\in\mathbb R^{32\times1024}$ |
| block | $S=\operatorname{LN}(X+P(X))$；$X'=\operatorname{LN}(S+F(S))$ | $Z=P^{-1}(P(X)+F_m(\operatorname{RMS}(P(X))))$；$X'=X+F_o(\operatorname{RMS}(Z))$ |
| FFN 非线性 | $F_t(u)=\phi(uW_{t,1}+b_{t,1})W_{t,2}+b_{t,2}$ | $F_t(u)=[(uW_{t,u}+b_{t,u})\odot\operatorname{SiLU}(uW_{t,g}+b_{t,g})]W_{t,d}+b_{t,d}$ |
| 读出与 logit | $h=\operatorname{LN}(p+r_B)$；$z=w^\top h+b$ | $h=[g;p_{\mathrm{cond}};f_{512}]$；$z=\operatorname{MLP}(h)$ |

v5 以全局路径和双空间交互重建主干，同时以三路读出替换原 Bucket Cross 融合接口。下面分别展开新增计算。
...
最终长残差连接的是 **当前 block 输入 $X$，跨越块内两段变换**，没有跨多个 block 的跳连。$Z$ 负责为 original-space FFN 提供条件，不直接替换最后加法中的残差输入。
...
```mermaid
flowchart TD
    subgraph BEFORE["修改前：v5"]
        OLD_A["固定均衡分组；31 Local + Global"] --> OLD_B["D1024；2 层双空间 SwiGLU；M704"] --> OLD_C["Global + Pool + Flatten → 2560 维"]
    end
    subgraph AFTER["修改后：v6；重点变化见节点"]
...
flowchart TD
    subgraph BEFORE["修改前：v3"]
        OLD_A["16 个语义输入 Token"] --> OLD_B["原主干 → Gated Pool"]
        OLD_A --> OLD_C["输入 Token 的 Bucket Cross"]
        OLD_B --> OLD_D["融合 LN → 768 维 → 线性 logit"]
        OLD_C --> OLD_D
    end
    subgraph AFTER["修改后：v7；重点变化见节点"]
...
令两版共同的融合表示为 $h_0\in\mathbb R^{768}$，预测接口由线性读出改为三层非线性头：

$$
\mathrm{v3}:~z=h_0w+b
\quad\longrightarrow\quad
\mathrm{v7}:~z=\operatorname{MLP}_{768\to2048\to2048\to256\to1}(h_0).
$$
...
```mermaid
flowchart TD
    subgraph BEFORE["修改前：v6"]
        OLD_A["BN/SENet 后 Raw 字段"] --> OLD_B["Raw 同时生成 Local 与 Global"] --> OLD_C["D512；双空间主干 M704 → 三路读出"]
    end
    subgraph AFTER["修改后：v8；重点变化见节点"]
...
**投影输入的前后变化。**令 $\Pi_t(u)=\operatorname{RMS}_t(\phi(uW_t+b_t))$，则：

$$
\mathrm{v6}:~l_t=\Pi_t(x_0[G_t])
\quad\longrightarrow\quad
\mathrm{v8}:~l_t=\Pi_t(x_2[G_t]),\qquad
x_2=\operatorname{MaskedDCN}_2(x_0).
$$

两版 Global 均读取 $x_0$。因此新增显式交叉的位置在字段压缩之前，改变的是 Local 所读取的视图。
...
```mermaid
flowchart TD
    subgraph BEFORE["修改前：v8"]
        OLD_A["Raw 字段 x0"] --> OLD_B["Masked DCN → Cross Local"]
        OLD_A --> OLD_C["Raw Global"]
        OLD_B --> OLD_D["双空间主干 → 三路 1536 维读出<br/>无 Cross Shortcut"]
        OLD_C --> OLD_D
    end
    subgraph AFTER["修改后：v9；重点变化见节点"]
...
**交叉公式改动前后。**v9 删除 v8 的 mask 支路，改用 Base 同型低秩 DCNM：

$$
\begin{aligned}
\mathrm{v8}:~x_{\ell+1}
&=\operatorname{LN}\!\left(x_\ell+x_0\odot
[((x_\ell V_\ell+b_{v,\ell})\odot m_\ell)U_\ell+b_{u,\ell}]\right),\\
\mathrm{v9}:~x_{\ell+1}
&=\operatorname{LN}\!\left(x_\ell+x_0\odot
[(x_\ell V_\ell+b_{v,\ell})U_\ell+b_{u,\ell}]\right).
\end{aligned}
$$

| 信息路径 | 修改前：v8 | 修改后：v9 |
|---|---|---|
| Local 投影输入 | $x_2[G_t]$ | $[x_0[G_t];x_2[G_t]]$，同时读取 Raw/Cross |
| Global 输入 | Raw：$x_0$ | Cross：$x_2$ |
| 末端旁路 | 无独立 Cross Shortcut | $s=\phi(\operatorname{BN}(x_2W_s+b_s))$ |
| 读出宽度 | $512+512+512=1536$ | $512+512+256+512=1792$ |

所以 v8→v9 不是只删除 mask；交叉算子、信息来源与末端融合同时变化。
...
flowchart TD
    subgraph BEFORE["修改前：v6"]
        OLD_A["语义 Local + Global；RMSNorm"] --> OLD_B["双空间 block；RMSNorm"] --> OLD_C["三路 1536 维读出 → 深任务头"]
    end
    subgraph AFTER["修改后：v10；重点变化见节点"]
...
| 位置 | 修改前：v6 | 修改后：v10 |
|---|---|---|
| Local / Global、block PreNorm、Final Norm | RMSNorm | LayerNorm；具体参数共享差异见 E3 |
| 末端接口 | $h=[g;p;f_{512}]\in\mathbb R^{1536}$ | $h=\operatorname{vec}(\operatorname{LN}(X_L))\in\mathbb R^{16384}$ |
| block 残差 | $X'=X+F_o(\operatorname{RMS}(Z))$ | $X'=X+F_o(\operatorname{LN}(Z))$；保留输入长残差 |

LayerNorm 同时替换 mixed 与 original 两处 PreNorm，因此 block 改为：
...
| 测试日 | Base | 设计起点：v6 | 上一编号版本：v9 | 当前模型：v10 | v10 − v9 | 说明 |
|---|---:|---:|---:|---:|---:|---|
| 2026-08-16 | 0.866960 | 0.866017 | 0.865254 | **未运行** | — | E3 的 0.866386 不登记为 v10 结果 |
...
flowchart TD
    subgraph BEFORE["修改前：v6"]
        OLD_A["原输入与双空间主干；RMSNorm"] --> OLD_B["Global / 条件 Pool / 压缩 Flatten"] --> OLD_C["拼接 1536 维 → 深任务头"]
    end
    subgraph AFTER["修改后：E2；重点变化见节点"]
...
**出发点。**v10 同时设计了 PureFlat 和 LayerNorm 两项改动，即使后续取得效果，也难以区分两者的作用。因此保留 E2 的输入、主干线性变换与任务头，只切换归一化实现；当前实测数值仍属于 E3，v10 尚未运行。
...
```mermaid
flowchart TD
    subgraph BEFORE["修改前：E2"]
        OLD_A["Local / Global 使用 RMSNorm"] --> OLD_B["两个 block 使用 RMSNorm"] --> OLD_C["Final RMSNorm → PureFlat 16384"]
    end
    subgraph AFTER["修改后：E3；重点变化见节点"]
...
flowchart TD
    subgraph BEFORE["修改前：E2"]
        OLD_A["相同语义组；逐 Token 切片"] --> OLD_B["D512；2 层双空间主干；M704"] --> OLD_C["PureFlat 16384 → 深任务头"]
    end
    subgraph AFTER["修改后：E2 Small；重点变化见节点"]
...
```mermaid
flowchart TD
    subgraph BEFORE["修改前：E2 Small"]
        OLD_A["10/20/1 Local；SENet 后三桶 Global"] --> OLD_B["三桶进入 D256 主干；Final RMSNorm"] --> OLD_C["PureFlat 8192 → 2048/2048/256 任务头"]
    end
    subgraph AFTER["修改后：E4；重点变化见节点"]
        A["三桶 BN"] --> SE["字段 SENet"]
        SE --> B["common/item 语义分组<br/>10 + 21 Local Token"]
        A --> C["SENet 前 common/item Global<br/>20740 → 256 → 256"]
        B --> D["31 Local + 1 Global<br/>32×256"]
        C --> D
        D --> E["2 层双空间 SwiGLU<br/>中间仍用 RMSNorm；M=704"]
        E --> F["Final LayerNorm<br/>Mean Pool → 256"]
        SE --> G["creative 独立支路<br/>238 → 256 → 32<br/>BN + 可训练 Swish"]
        F --> H["Concat：288"]
        G --> H
...
6. Global 输入由 SENet 后改到 SENet 前：直接使用 BN 后的 common/item；Local 与 creative 仍读取各自 SENet 后的表示。

**读出公式改动前后。**Small 让三个桶都进入 Mixer，并将全部 token 展平：

$$
h_{\mathrm{Small}}=\operatorname{vec}(\operatorname{RMS}(X_L))
\in\mathbb R^{8192},\qquad
z_{\mathrm{Small}}=\operatorname{MLP}_{8192\to2048\to2048\to256\to1}(h_{\mathrm{Small}}).
$$

E4 改为 common/item 主干均值与 creative 旁路末端拼接，读出降到 $256+32=288$ 维：
...
**分析。**E4 将参数进一步降至 **80.739M**，但首日比 Small 下降万1.77。creative 路由、Local 分组、Global 输入来源、均值池化、Final LN 和小任务头同时改变，现有结果只能说明这套完整简化组合牺牲了一部分 AUC，不能定位单个原因。
...
```mermaid
flowchart TD
    subgraph BEFORE["修改前：mature_v1"]
        OLD_A["粗组 Local + Global；D256"] --> OLD_B["3 层 pSwiGLU；M896"] --> OLD_C["Mean 256 + creative 32 → 256/128 任务头"]
    end
    subgraph AFTER["修改后：mature_v2；重点变化见节点"]
...
```mermaid
flowchart TD
    subgraph BEFORE["修改前：mature_v2"]
        OLD_A["相同 D384 输入"] --> OLD_B["3 层 pSwiGLU；M1344"] --> OLD_C["Mean 384 + creative 48 → 256/128 任务头"]
    end
    subgraph AFTER["修改后：mature_v3；重点变化见节点"]
...
**思路来源。**pSiLU 是在公司成熟 block 上做的内部减参变体。它从 SwiGLU 的双上投影结构出发，删除独立 gate 上投影，将保留的 value/up 输出直接送入 SiLU，取消双分支乘积；保留双 RMSNorm 和原残差位置。

对应源码：[mature_v4 pSiLU](/Users/goku/Documents/Codex/RSA_code_0816/src/models/rankmixer/cvr_senet_mature_rankmixer_v4.py:1370)。

```mermaid
flowchart TD
    subgraph BEFORE["修改前：mature_v1；v2 为未测容量参考"]
        OLD_A["D256 / M896；较窄分支"] --> OLD_B["3 层双上投影 pSwiGLU"] --> OLD_C["Mean 256 + creative 32 → 256/128 任务头"]
    end
    subgraph AFTER["修改后：mature_v4；重点变化见节点"]
...
**FFN 公式改动前后。**令 $Z=P(X)$、$Q=\operatorname{LN}(Z)$。mature_v1/v2/v3 的双路 pSwiGLU 隐层为：

$$
H_{\mathrm{SwiGLU}}
=\operatorname{RMS}_h\!\left(
\operatorname{SiLU}(QW_g+b_g)\odot(QW_u+b_u)\right).
$$

mature_v4 删除独立 gate 投影 $W_g,b_g$，让保留的 value/up 投影直接经过 SiLU；下投影、双 RMSNorm 和 mixed-space 残差位置不变：
...
```mermaid
flowchart TD
    subgraph BEFORE["修改前：mature_v4"]
        OLD_A["7 个粗组；每组产生多个 Token"] --> OLD_B["共享组内字段 → Linear + GELU + BN"] --> OLD_C["31 Local + Global → 原 pSiLU 主干与末端"]
    end
    subgraph AFTER["修改后：mature_v5；重点变化见节点"]
...
**Tokenizer 公式改动前后。**v4 的一个粗组 $g$ 同时产生 $k_g$ 个 token：

$$
X_g=\operatorname{reshape}_{k_g\times D}
\left(\operatorname{BN}_g\!\left(\phi(\widetilde x_gW_g+b_g)\right)\right),
\qquad W_g\in\mathbb R^{17n_g\times k_gD}.
$$

这些 token 共享同一组输入字段，但输出列的参数各自独立。v5 将输入拆成更细的语义组，每组只产生一个 token，并去掉投影后的 GELU：
...
每个组有独立投影和独立 BN，统计沿 batch 维计算。**当前 v5 在 Linear 与 BN 之间没有 GELU**，而 v4 的粗组投影使用 GELU。同时，投影权重由 v4 默认的 Glorot uniform 改成标准差为 $1/\sqrt{17|G_t|}$ 的正态初始化。因此这一处理同时改变字段组织、投影输入共享范围、参数量、投影激活和初始化，结论应归于整个 tokenizer 的替换。
...
| 测试日 | Base | 路线起点：mature_v1 | 对比模型：mature_v4 | 当前模型：mature_v5 | v5 − v4 | v5 − mature_v1 | v5 − Base |
|---|---:|---:|---:|---:|---:|---:|---:|
| 2026-08-16 | 0.866960 | 0.866934 | 0.866206 | 0.866289 | **万0.83** | −万6.45 | −万6.71 |

**分析。**mature_v5 比 v4 小幅提升万0.83，但仍比 mature_v1 低万6.45、比 Base 低万6.71。参数量按当前代码和 args 记为 **135.775M**。v4→v5 保持主干与末端分支不变，但 tokenizer 同时改变语义组织、投影共享范围、激活、初始化和参数量，因此结果归属于完整 tokenizer 替换。
...
| AUC 与训练配置对照 | 见 6.2 的 Base / v6 / mature_v1 同日表 | 见同表 | 属于完整方案比较；当前保存的学习率衰减下限亦不同（见 2.2） |
...
**双流与固定 Mixing 路线的结构区别。**UniMixer 还引入 SiameseNorm，初始化 $x_0=y_0=X_0$，每个 block 按源码递推为：

$$
\begin{aligned}
s_\ell&=x_\ell+\operatorname{RMS}(y_\ell),\\
u_\ell&=\operatorname{pSwiGLU}\!\left(
\operatorname{RMS}(s_\ell+\operatorname{Mix}(s_\ell))\right),\\
x_{\ell+1}&=\operatorname{RMS}(x_\ell+u_\ell),\qquad
y_{\ell+1}=y_\ell+u_\ell,\\
h&=\operatorname{vec}\!\left(x_L+\operatorname{RMS}(y_L)\right).
\end{aligned}
$$

与 v6 的单流 $X\to P\to F_m\to P^{-1}\to F_o$ 相比，这里同时更换可学习 Mix、双流状态更新与最终 PureFlat；不存在 v6 那种 Reverting 后的第二套 FFN。
...
| 方案与窗口 | 测试日期 | 天数 | 同窗口 Base 平均 AUC | 当前模型平均 AUC | 当前 − Base |
|---|---|---:|---:|---:|---:|
| 初始 v1（无代码） | 2026-07-02～2026-07-09 | 8 | 0.866490 | 0.861362 | −千5.128 |
| BN v1 | 2026-07-02～2026-07-05 | 4 | 0.865836 | 0.863170 | −千2.666 |
| v3 | 2026-07-02～2026-07-08 | 7 | 0.866215 | 0.865005 | −千1.210 |
| v5：历史前三日窗口 | 2026-07-02～2026-07-04 | 3 | 0.8657437 | 0.8650433 | −万7.00 |
| v6 | 2026-08-16～2026-08-19 | 4 | 0.868401 | 0.867495 | −万9.06 |
| E2 | 2026-08-16～2026-08-18 | 3 | 0.867912 | 0.867497 | −万4.15 |
| mature_v1 | 2026-08-16～2026-08-17 | 2 | 0.867414 | 0.867368 | −万0.46 |

这些窗口长度与日期不同，用于描述各自结果；候选之间的直接比较应使用下表的共同日期。差值先按 Excel 原始值计算，再按千/万口径四舍五入；展示的均值也单独四舍五入。

### A.5 关键同日差值复算

#### A.5.1 单日比较

| 测试日 | Base | 对比模型 | 当前模型 | 当前 − 对比 | 当前 − Base |
|---|---:|---:|---:|---:|---:|
| 2026-07-02 | 0.864538 | 初始 v1：0.858606 | BN v1：0.862033 | 千3.427 | −千2.505 |
| 2026-07-02 | 0.864538 | BN v1：0.862033 | v2：0.862690 | 万6.57 | −千1.848 |
| 2026-07-02 | 0.864538 | v2：0.862690 | v3：0.862991 | 万3.01 | −千1.547 |
| 2026-07-02 | 0.864538 | v3：0.862991 | v4：0.862709 | −万2.82 | −千1.829 |
| 2026-07-02 | 0.864538 | v4：0.862709 | v5：0.863747 | 千1.038 | −万7.91 |
| 2026-08-16 | 0.866960 | v5：0.864163 | v6：0.866017 | 千1.854 | −万9.43 |
| 2026-08-16 | 0.866960 | v6：0.866017 | v7：0.865866 | −万1.51 | −千1.094 |
| 2026-08-16 | 0.866960 | v7：0.865866 | v8：0.866615 | 万7.49 | −万3.45 |
| 2026-08-16 | 0.866960 | v6：0.866017 | v8：0.866615 | 万5.98 | −万3.45 |
| 2026-08-16 | 0.866960 | v8：0.866615 | v9：0.865254 | −千1.361 | −千1.706 |
| 2026-08-16 | 0.866960 | E2：0.866562 | E3：0.866386 | −万1.76 | −万5.74 |
| 2026-08-16 | 0.866960 | E2：0.866562 | Small：0.866510 | −万0.52 | −万4.50 |
| 2026-08-16 | 0.866960 | Small：0.866510 | E4：0.866333 | −万1.77 | −万6.27 |
| 2026-08-16 | 0.866960 | v6：0.866017 | UniMixer v1：0.865662 | −万3.55 | −千1.298 |
| 2026-08-16 | 0.866960 | mature_v1：0.866934 | mature_v4：0.866206 | −万7.28 | −万7.54 |
| 2026-08-16 | 0.866960 | mature_v4：0.866206 | mature_v5：0.866289 | 万0.83 | −万6.71 |
| 2026-08-16 | 0.866960 | mature_v1：0.866934 | mature_v5：0.866289 | −万6.45 | −万6.71 |

同日结果用于复算观察差值；v7/v6 等非直接设计继承关系、v9/E4 等联合修改，均不能解释为单模块净收益。各版动机与归因边界见正文。
...
| 2026-07-03 | 0.865633 | 0.864324 | 0.864929 | 万6.05 | −万7.04 |
| 2026-07-04 | 0.867060 | 0.865902 | 0.866454 | 万5.52 | −万6.06 |
| 共同 3 日均值 | 0.8657437 | 0.8644057 | 0.8650433 | 万6.38 | −万7.00 |
...
| 2026-08-17 | 0.867867 | 0.867088 | 0.867488 | 万4.00 | −万3.79 |
| 2026-08-18 | 0.868909 | 0.867996 | 0.868440 | 万4.44 | −万4.69 |
| 共同 3 日均值 | 0.8679120 | 0.8670337 | 0.8674967 | 万4.63 | −万4.15 |

**v6 与 mature_v1 的共同测试日**

| 测试日 | Base | 对比模型：v6 | 当前模型：mature_v1 | mature_v1 − v6 | mature_v1 − Base |
|---|---:|---:|---:|---:|---:|
| 2026-08-16 | 0.866960 | 0.866017 | 0.866934 | 万9.17 | −万0.26 |
| 2026-08-17 | 0.867867 | 0.867088 | 0.867801 | 万7.13 | −万0.66 |
| 共同 2 日均值 | 0.8674135 | 0.8665525 | 0.8673675 | 万8.15 | −万0.46 |
